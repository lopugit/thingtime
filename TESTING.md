...
- [ ] On `/thing/:diagnostic-id`, click Reveal, enter the current owning admin's
	      ...
	      admin or non-admin receives no value, and five attempts in 15 minutes hit
	      the fixed fail-closed ceiling. Success and every error carry
	      `private, no-store` and `Pragma: no-cache`.
- [ ] Throw diagnostics containing a 24-hex password/token/authorization value,
	      a connection-string credential, a sensitive URL query value, and an
...
- [ ] Force each typed migration operator failure once: the response uses the
      ...
      includes only registered migration ids and aggregate counts, and keeps
	      ...
	      the public response. Only explicitly contextual ObjectIds may enter the
	      protected diagnostic reveal table; all other private context stays in
