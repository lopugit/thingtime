# Graph Report - /Users/lopu/.codex/tmp/thingtime-exact-sync-inventory.S7g78U/worktree  (2026-09-01)

## Corpus Check
- cluster-only mode — file stats not available

## Summary
- 766 nodes · 1647 edges · 54 communities (39 shown, 15 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 19 edges (avg confidence: 0.62)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7bfa61bd`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- [[_COMMUNITY_Develop Preview Deployment|Develop Preview Deployment]]
- [[_COMMUNITY_Graphify Snapshot Management|Graphify Snapshot Management]]
- [[_COMMUNITY_Promotion PR Changelog|Promotion PR Changelog]]
- [[_COMMUNITY_All Branch Build Checks|All Branch Build Checks]]
- [[_COMMUNITY_PR Manager Workflow|PR Manager Workflow]]
- [[_COMMUNITY_Promotion Stack Selection|Promotion Stack Selection]]
- [[_COMMUNITY_Workflow Control Plane|Workflow Control Plane]]
- [[_COMMUNITY_PR Status Reporting|PR Status Reporting]]
- [[_COMMUNITY_Promotion Reservation Handling|Promotion Reservation Handling]]
- [[_COMMUNITY_Promotion PR Automation|Promotion PR Automation]]
- [[_COMMUNITY_Promotion Graphify Refresh|Promotion Graphify Refresh]]
- [[_COMMUNITY_CodeQL PR Backfill|CodeQL PR Backfill]]
- [[_COMMUNITY_Feature Promotion Recovery|Feature Promotion Recovery]]
- [[_COMMUNITY_Conflict Routing Contract|Conflict Routing Contract]]
- [[_COMMUNITY_Main Develop Sync|Main Develop Sync]]
- [[_COMMUNITY_Promotion Worker Contract|Promotion Worker Contract]]
- [[_COMMUNITY_Lopu Agent Workflows|Lopu Agent Workflows]]
- [[_COMMUNITY_Rebase Conflict Tests|Rebase Conflict Tests]]
- [[_COMMUNITY_Rebase Round Preparation|Rebase Round Preparation]]
- [[_COMMUNITY_Promotion Worker Script|Promotion Worker Script]]
- [[_COMMUNITY_Main Develop Synchronization|Main Develop Synchronization]]
- [[_COMMUNITY_Promotion Worker Routing|Promotion Worker Routing]]
- [[_COMMUNITY_Workflow CI Jobs|Workflow CI Jobs]]
- [[_COMMUNITY_Promotion Checkpoint Recovery|Promotion Checkpoint Recovery]]
- [[_COMMUNITY_Promotion Dispatch Queue|Promotion Dispatch Queue]]
- [[_COMMUNITY_Graphify Snapshot Staging|Graphify Snapshot Staging]]
- [[_COMMUNITY_Develop Preview Infrastructure|Develop Preview Infrastructure]]
- [[_COMMUNITY_Feature Stack Planning|Feature Stack Planning]]
- [[_COMMUNITY_Credential Vault|Credential Vault]]
- [[_COMMUNITY_Rebase Start Script|Rebase Start Script]]
- [[_COMMUNITY_Claude Credential Classification|Claude Credential Classification]]
- [[_COMMUNITY_Promotion Source Verification|Promotion Source Verification]]
- [[_COMMUNITY_Vercel Configuration|Vercel Configuration]]
- [[_COMMUNITY_CodeQL Branch Analysis|CodeQL Branch Analysis]]
- [[_COMMUNITY_Electron PR Contract|Electron PR Contract]]
- [[_COMMUNITY_Rebase Index Fingerprints|Rebase Index Fingerprints]]
- [[_COMMUNITY_Rebase Ownership Routing|Rebase Ownership Routing]]
- [[_COMMUNITY_Instruction Conflict Resolution|Instruction Conflict Resolution]]
- [[_COMMUNITY_Instruction Conflict Tests|Instruction Conflict Tests]]
- [[_COMMUNITY_Commander Release Script|Commander Release Script]]
- [[_COMMUNITY_PM2 Port Management|PM2 Port Management]]
- [[_COMMUNITY_Protected Branch Merging|Protected Branch Merging]]
- [[_COMMUNITY_Electron Release Workflow|Electron Release Workflow]]
- [[_COMMUNITY_Graphify CLI|Graphify CLI]]
- [[_COMMUNITY_Lopu Agent Action|Lopu Agent Action]]
- [[_COMMUNITY_Delivery Messaging Rules|Delivery Messaging Rules]]
- [[_COMMUNITY_iOS Release Flow|iOS Release Flow]]
- [[_COMMUNITY_Fundamentals|Fundamentals]]
- [[_COMMUNITY_CodeQL PR Handoff|CodeQL PR Handoff]]
- [[_COMMUNITY_Legacy Conflict Resolver|Legacy Conflict Resolver]]
- [[_COMMUNITY_Develop Preview Script|Develop Preview Script]]
- [[_COMMUNITY_Electron PR Release|Electron PR Release]]
- [[_COMMUNITY_Testing Checklist|Testing Checklist]]

## God Nodes (most connected - your core abstractions)
1. `selfTest()` - 49 edges
2. `runPromotion()` - 36 edges
3. `failureDetail()` - 28 edges
4. `deploy()` - 26 edges
5. `runSelfTest()` - 24 edges
6. `main()` - 20 edges
7. `orphanedMergeHydrationIntegrationTest()` - 19 edges
8. `main()` - 15 edges
9. `repoFlag()` - 15 edges
10. `buildMode()` - 14 edges

## Surprising Connections (you probably didn't know these)
- `PR #492 Load Lopu credentials from Thingtime vault` --references--> `Lopu agent composite action`  [INFERRED]
  PRs/492-codex-lopu-credential-vault-controller-load-lopu-credentials-from-thingtime-vault.md → .github/actions/lopu-agent/action.yml
- `Thingtime AI Instructions` --semantically_similar_to--> `Thingtime AI Instructions`  [EXTRACTED] [semantically similar]
  AI_ALL.md → AGENTS.md
- `Thingtime AI Instructions` --semantically_similar_to--> `Thingtime AI Instructions`  [EXTRACTED] [semantically similar]
  CLAUDE.md → AGENTS.md
- `Lopu Automation Changes` --conceptually_related_to--> `Lopu Principal Repository Manager`  [INFERRED]
  CHANGELOG.md → README.md
- `GitHub PR Policy` --conceptually_related_to--> `github-actions CI Control Plane`  [INFERRED]
  AGENTS.md → README.md

## Import Cycles
- None detected.

## Hyperedges (group relationships)
- **Lopu Claude and Codex backend selection** — _github_actions_lopu_agent_action_lopu_agent_action, _github_actions_lopu_agent_action_claude_code_action, _github_actions_lopu_agent_action_codex_action, _github_actions_lopu_agent_action_lopu_credential_vault_mjs [EXTRACTED 0.92]
- **Control-plane Contract Validation** — readme_workflow_control_plane_contract, _github_workflows_control_plane_ci_workflow, _github_workflows_control_plane_ci_verify_job, _github_workflows_control_plane_ci_contract_advisories_job [EXTRACTED 0.90]
- **Terminal Merge Safety Gate** — prs_506_codex_auto_merge_main_develop_sync_pr_trusted_terminal_merger, prs_506_codex_auto_merge_main_develop_sync_pr_thingtime_repository, prs_506_codex_auto_merge_main_develop_sync_pr_sync_main_into_develop_pr, prs_506_codex_auto_merge_main_develop_sync_pr_develop_branch, prs_506_codex_auto_merge_main_develop_sync_pr_main_branch, prs_506_codex_auto_merge_main_develop_sync_pr_exact_head_rest_merge [EXTRACTED 0.90]
- **Validation Check Suite** — prs_506_codex_auto_merge_main_develop_sync_pr_pr_506, prs_506_codex_auto_merge_main_develop_sync_pr_self_tests, prs_506_codex_auto_merge_main_develop_sync_pr_graphify_extraction, prs_506_codex_auto_merge_main_develop_sync_pr_local_codex_proxy [EXTRACTED 0.85]
- **Operational Proof Gate** — prs_506_codex_auto_merge_main_develop_sync_pr_pr_475, prs_506_codex_auto_merge_main_develop_sync_pr_protected_lopu_entrypoint, prs_506_codex_auto_merge_main_develop_sync_pr_sync_main_into_develop_pr, prs_506_codex_auto_merge_main_develop_sync_pr_main_branch, prs_506_codex_auto_merge_main_develop_sync_pr_develop_branch [EXTRACTED 0.90]
- **Canonical Instruction Alias Set** — agents_ai_all_canonical_instruction_file, agents_root_instruction_symlinks, ai_all_thingtime_ai_instructions, claude_thingtime_ai_instructions [EXTRACTED 1.00]
- **Thingtime Data and API Contract** — agents_fundamentals, agents_thingtime_api, agents_mongodb_collections, agents_api_endpoint_registration [EXTRACTED 0.95]
- **Control Plane Lopu Graphify Flow** — readme_lopu_manager, _github_workflows_resolve_pr_conflicts_workflow, _github_workflows_resolve_pr_conflicts_graphify_refresh, readme_graphify_router, changelog_graphify_cas_changes [INFERRED 0.86]
- **Shared CI provider routing contract** — github_workflows_ci_provider_router_workflow, github_workflows_promote_develop_to_main_route, github_workflows_promote_features_to_main_route, github_workflows_resolve_pr_conflicts_workflow [EXTRACTED 0.80]
- **develop→main promotion and back-sync automation** — github_workflows_promote_develop_to_main_workflow, github_workflows_promote_features_to_main_workflow, github_workflows_sync_main_into_develop_workflow, github_workflows_resolve_pr_conflicts_workflow, github_scripts_promote_features_to_main, github_scripts_promotion_pr_changelog [EXTRACTED 0.85]
- **Develop PR Preview Controller Flow** — github_workflows_develop_pr_preview_workflow, github_workflows_develop_pr_preview_dispatch_job, github_workflows_develop_pr_preview_repository_dispatch, github_workflows_develop_pr_preview_controller_event, github_workflows_develop_pr_preview_controller_job, github_workflows_develop_pr_preview_deploy_script [EXTRACTED 0.95]

## Communities (54 total, 15 thin omitted)

### Community 0 - "Develop Preview Deployment"
Cohesion: 0.06
Nodes (95): ACTIVE_STATES, assertCurrentPullRequest(), assertRepositoryDispatchSource(), assertTrustedPrincipal(), assertTrustedPullRequest(), assertTrustedPullRequestStack(), assertVercelConfiguration(), assertWildcardFallbackRuntimes() (+87 more)

### Community 1 - "Graphify Snapshot Management"
Cohesion: 0.09
Nodes (54): activateSnapshot(), artifactHash(), baselineNodeCount(), compareSnapshotQuality(), computeSourceFingerprint(), copyPortableFiles(), ensureSnapshot(), fail() (+46 more)

### Community 2 - "Promotion PR Changelog"
Cohesion: 0.11
Nodes (42): associatedPr(), bodyFile(), buildComment(), buildSection(), CFG, computeDelta(), computeMissingLabels(), contentIndex (+34 more)

### Community 3 - "All Branch Build Checks"
Cohesion: 0.16
Nodes (36): assertAllBranchWorkflowContract(), BASE_BRANCHES, buildMode(), checkMode(), completeRefspecs, countLeadingFailureMarkers(), doctorCommitMode(), doctorRecordMode() (+28 more)

### Community 4 - "PR Manager Workflow"
Cohesion: 0.06
Nodes (35): ai-merge-paused Stop Label, PR Conflict Resolution, Detector Handoff, Post-merge Graphify Refresh, Lopu PR Manager Workflow, API Suite Job, Build Typecheck Ratchet Unit Tests Job, Product Contract Advisories (+27 more)

### Community 5 - "Promotion Stack Selection"
Cohesion: 0.09
Nodes (33): botCommentsByLatestEvent(), clearSourceStandAside(), computePicks(), dependentMembersAfter(), externalStackPromotionState(), findBotPromotionRetirement(), groupFailureMessages(), groupKeyFor() (+25 more)

### Community 6 - "Workflow Control Plane"
Cohesion: 0.13
Nodes (27): acceptsBotRoutingProof(), actions, AI_RUNTIME_YAML, appReentryDisposition(), assertAdminLoader(), assertAdminModelRouting(), assertAdminTransportCap(), assertAdminWaterfallGrammar() (+19 more)

### Community 7 - "PR Status Reporting"
Cohesion: 0.16
Nodes (23): argument(), branchCode(), classifyInventory(), count(), dateParts(), escapeHtml(), FACT_LABEL_ORDER, formatZonedTimestamp() (+15 more)

### Community 8 - "Promotion Reservation Handling"
Cohesion: 0.15
Nodes (27): applyPicks(), buildPromotionPlanContext(), checkoutRemoteBranch(), createPromotionReservation(), createPromotionReviewCheckpoint(), ensureCommitAvailable(), ensureRemoteBranchAvailable(), expectedReservationTrailers() (+19 more)

### Community 9 - "Promotion PR Automation"
Cohesion: 0.18
Nodes (26): cancelPromotionRetirement(), closeRedundantPass(), createPromotionPr(), ensurePromotionLabel(), ensureSourceLineageReviewLabel(), exactBranchDeleteWithActionsToken(), finalizeAiPromotionMetadata(), finalizeSourceLineageMetadata() (+18 more)

### Community 10 - "Promotion Graphify Refresh"
Cohesion: 0.12
Nodes (21): assert_control_metadata_unchanged(), assert_tool_boundary(), current_refs_hash(), emit(), fail(), GIT_ATTR_NOSYSTEM, GIT_CONFIG_COUNT, GIT_CONFIG_GLOBAL (+13 more)

### Community 11 - "CodeQL PR Backfill"
Cohesion: 0.21
Nodes (22): ACTIVE_RUN_STATUSES, activePrHeadKeys(), analysisKey(), analysisSnapshotForPullRequest(), commandFailureText(), completeAnalysisKeys(), dispatchAnalysisWithInput(), flattenSlurp() (+14 more)

### Community 12 - "Feature Promotion Recovery"
Cohesion: 0.13
Nodes (18): CFG, checkpointRecoveryDisposition(), encodePromotionAttestation(), env(), EXEC_OPTS, flag(), isExactPausedPromotionSnapshot(), listSourceIssueComments() (+10 more)

### Community 13 - "Conflict Routing Contract"
Cohesion: 0.17
Nodes (18): aiRuntimeSourceFiles(), assertAdminLoader(), assertAdminModelRouting(), assertRoute(), assertWorkflowSource(), decodeBatch(), encodeBatch(), LOPU_ACTION_URL (+10 more)

### Community 14 - "Main Develop Sync"
Cohesion: 0.20
Nodes (14): assertHeadContainsMain(), assertSyncPullRequestShape(), exactSha(), EXPECTED_MERGE_REJECTIONS, GitHubRequestError, headContainsMain(), mergeStandingSyncPullRequest(), notice() (+6 more)

### Community 15 - "Promotion Worker Contract"
Cohesion: 0.12
Nodes (16): BASE_REF, BASE_SHA, GITHUB_OUTPUT, PLAN_HASH, PROMOTION_BRANCH, reject_lineage_mismatch(), require_lineage_replay(), RESERVATION_SHA (+8 more)

### Community 16 - "Lopu Agent Workflows"
Cohesion: 0.13
Nodes (15): classify-claude-credential-failure.mjs, anthropics/claude-code-action, openai/codex-action, Lopu agent composite action, lopu-credential-vault.mjs, graphify-cas.mjs, prepare-round.sh, Lopu rebase conflict round composite action (+7 more)

### Community 17 - "Rebase Conflict Tests"
Cohesion: 0.28
Nodes (14): actionPath, copyTrustedTree(), extractVerifierScript(), filesUnder(), hashNamedFiles(), hashRebaseState(), hashTrustedTree(), makeStoppedRebase() (+6 more)

### Community 18 - "Rebase Round Preparation"
Cohesion: 0.26
Nodes (14): assert_safe_regular_text_conflict(), clear_scratch(), emit(), emit_paths(), has_coherent_zdiff3_markers(), hash_index_entries(), hash_rebase_state(), rebase_in_progress() (+6 more)

### Community 19 - "Promotion Worker Script"
Cohesion: 0.35
Nodes (12): classify_source_lineage(), emit(), emit_paths(), fail(), prepare(), require_environment(), require_reservation(), secure_git_environment() (+4 more)

### Community 20 - "Main Develop Synchronization"
Cohesion: 0.15
Nodes (14): Clean Synchronization Lane, develop Branch, GitHub Exact-Head REST Merge, Semantic Graphify Extraction, Local Codex Proxy, Lopu Conflict Resolver, main Branch, PR #475 (+6 more)

### Community 21 - "Promotion Worker Routing"
Cohesion: 0.17
Nodes (11): action, allBranchWorkflow, developPromotionWorkflow, featurePromotionWorkflow, graphify, lopuAgent, mainDevelopSyncWorkflow, promoter (+3 more)

### Community 22 - "Workflow CI Jobs"
Cohesion: 0.20
Nodes (11): comment-contract-advisories job, contract-advisories job, verify job, Workflow control-plane CI, ci-provider-router.yml, merge-main-develop-sync-pr.mjs, route job, sync job (+3 more)

### Community 23 - "Promotion Checkpoint Recovery"
Cohesion: 0.27
Nodes (11): attestationMatches(), exactCheckpointPush(), inspectPromotionReviewCheckpoint(), isObjectId(), latestBotPromotionAttestationEvents(), liveRefShaWithActionsToken(), parsePromotionResolutionAttestations(), recoverPromotionReviewCheckpoint() (+3 more)

### Community 24 - "Promotion Dispatch Queue"
Cohesion: 0.24
Nodes (10): buildPromotionDispatchRequest(), dispatchPromotionResolution(), exactReservationDeleteArgs(), exactReservationPushArgs(), promotionBody(), promotionDispatchArgs(), queueTrustedPromotionWorker(), redispatchPromotionReservation() (+2 more)

### Community 25 - "Graphify Snapshot Staging"
Cohesion: 0.40
Nodes (9): addExisting(), filesUnder(), git(), LEGACY_ROOT, PORTABLE, restoreTrackedFromHead(), selfTest(), stageGraphifySnapshots() (+1 more)

### Community 26 - "Develop Preview Infrastructure"
Cohesion: 0.20
Nodes (10): actions/checkout v4.4.0, develop-pr-preview-controller event, Publish or reconcile develop S3 preview job, .github/scripts/deploy-develop-pr-preview.mjs, Dispatch trusted default-branch controller job, GitHub repository dispatch API, VERCEL_DEVELOP_DEPLOY_TOKEN, vercel-develop-pr-control environment (+2 more)

### Community 27 - "Feature Stack Planning"
Cohesion: 0.64
Nodes (7): canonicalFeatureStackPlan(), decodeFeatureStackPlan(), exactKeys(), featureStackId(), git(), selfTest(), verifyFeatureStackHistory()

### Community 28 - "Credential Vault"
Cohesion: 0.61
Nodes (7): exportBundle(), fetchBundle(), legacyCredentials(), main(), required(), safeCache(), validate()

### Community 29 - "Rebase Start Script"
Cohesion: 0.46
Nodes (7): emit(), emit_paths(), rebase_in_progress(), secure_git_environment(), start.sh script, usage(), write_conflicts()

### Community 30 - "Claude Credential Classification"
Cohesion: 0.48
Nodes (6): CAPACITY_PATTERNS, classifyClaudeCredentialFailure(), collectStrings(), CREDENTIAL_PATTERNS, main(), selfTest()

### Community 31 - "Promotion Source Verification"
Cohesion: 0.33
Nodes (6): fail(), GIT_ATTR_NOSYSTEM, GIT_CONFIG_GLOBAL, GIT_CONFIG_NOSYSTEM, GIT_CONFIG_SYSTEM, verify-promotion-source-authority.sh script

### Community 32 - "Vercel Configuration"
Cohesion: 0.33
Nodes (5): framework, git, deploymentEnabled, ignoreCommand, $schema

### Community 33 - "CodeQL Branch Analysis"
Cohesion: 0.40
Nodes (5): Analyze Job, github/codeql-action/analyze, github/codeql-action/init, Lopu CodeQL All Branches, Select One Analysis Owner

### Community 34 - "Electron PR Contract"
Cohesion: 0.50
Nodes (4): assertPrReleaseContract(), count(), here, workflow

### Community 35 - "Rebase Index Fingerprints"
Cohesion: 0.80
Nodes (4): rawIndexHash(), runGit(), semanticIndexHash(), sha256()

### Community 36 - "Rebase Ownership Routing"
Cohesion: 0.83
Nodes (3): assert_owner(), assert_stack(), rebase-ownership-routing-contract.sh script

### Community 39 - "Commander Release Script"
Cohesion: 0.67
Nodes (3): build_and_run.sh, Commander App Release workflow, release-version.mjs

## Knowledge Gaps
- **168 isolated node(s):** `BASE_BRANCHES`, `completeRefspecs`, `MERGE_CONFIG`, `CAPACITY_PATTERNS`, `CREDENTIAL_PATTERNS` (+163 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **15 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `promote: replay merged develop PRs` connect `Promotion PR Changelog` to `Feature Promotion Recovery`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **What connects `BASE_BRANCHES`, `completeRefspecs`, `MERGE_CONFIG` to the rest of the system?**
  _168 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Develop Preview Deployment` be split into smaller, more focused modules?**
  _Cohesion score 0.06332842415316642 - nodes in this community are weakly interconnected._
- **Should `Graphify Snapshot Management` be split into smaller, more focused modules?**
  _Cohesion score 0.09175920514319112 - nodes in this community are weakly interconnected._
- **Should `Promotion PR Changelog` be split into smaller, more focused modules?**
  _Cohesion score 0.10631229235880399 - nodes in this community are weakly interconnected._
- **Should `PR Manager Workflow` be split into smaller, more focused modules?**
  _Cohesion score 0.06050420168067227 - nodes in this community are weakly interconnected._
- **Should `Promotion Stack Selection` be split into smaller, more focused modules?**
  _Cohesion score 0.09090909090909091 - nodes in this community are weakly interconnected._