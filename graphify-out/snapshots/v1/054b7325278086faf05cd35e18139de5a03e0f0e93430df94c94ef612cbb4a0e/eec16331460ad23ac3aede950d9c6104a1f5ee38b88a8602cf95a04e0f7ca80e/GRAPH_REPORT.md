...
- `CI Control Plane` --references--> `Vercel Deployment Kill Switch`  [EXTRACTED]
...
- `Thingtime AI Instructions` --semantically_similar_to--> `Thingtime AI Instructions`  [EXTRACTED] [semantically similar]
  CLAUDE.md → AGENTS.md
- `Control-plane Changelog` --references--> `Thingtime Electron PR Release`  [INFERRED]
  CHANGELOG.md → .github/workflows/electron-pr-release.yml
- `CI Control Plane` --references--> `Electron PR Release Workflow`  [EXTRACTED]
  README.md → .github/workflows/electron-pr-release.yml
- `CI Control Plane` --references--> `Lopu PR Manager`  [EXTRACTED]
...
- **Control-Plane Contract Validation** — github_workflows_control_plane_ci_verify, github_workflows_control_plane_ci_contract_advisories, github_scripts_workflow_control_plane_contract [EXTRACTED 0.93]
- **Lopu Repository Automation** — github_workflows_resolve_pr_conflicts, github_actions_lopu_agent, readme_ci_control_plane [INFERRED 0.82]
...
Cohesion: 0.08
Nodes (56): ACTIVE_STATES, assertCurrentPullRequest(), assertPrebuiltOutput(), assignAlias(), boundedInteger(), cleanupDeploymentIssue(), cleanupEnvironment(), commentBody() (+48 more)
...
Cohesion: 0.09
Nodes (38): Lopu Agent Composite Action, Graphify Snapshot Router, acceptsBotRoutingProof(), actions, AI_RUNTIME_YAML, appReentryDisposition(), assertAdminLoader(), assertAdminModelRouting() (+30 more)
...
Cohesion: 0.67
Nodes (3): Control-plane Changelog, Thingtime Electron PR Release, Electron App Release
...
## Knowledge Gaps
- **205 isolated node(s):** `BASE_BRANCHES`, `completeRefspecs`, `MERGE_CONFIG`, `CAPACITY_PATTERNS`, `CREDENTIAL_PATTERNS` (+200 more)
...
- **Why does `CI Control Plane` connect `refresh-promotion-graphify.sh` to `deploy-develop-pr-preview.mjs`, `deploy-admin-pr-previews.mjs`?**
  _High betweenness centrality (0.020) - this node is a cross-community bridge._
- **Why does `Contract Advisories Job` connect `refresh-promotion-graphify.sh` to `deploy-develop-pr-preview.mjs`, `deploy-admin-pr-previews.mjs`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Why does `promote: replay merged develop PRs` connect `build-all-branch.mjs` to `codeql-open-pr-backfill.mjs`?**
