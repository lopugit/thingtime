...
- `Resolve release metadata version` --calls--> `Commander release-version.mjs`  [EXTRACTED]
  .github/workflows/commander-release.yml → Commander/script/release-version.mjs
- `Build Commander bundle` --calls--> `Commander build_and_run.sh`  [EXTRACTED]
...
- `Lopu PR Manager Workflow` --conceptually_related_to--> `Lopu PR Manager`  [INFERRED]
...
- **macOS Developer ID signing, notarization, and stapling flow** — github_workflows_commander_release_import_signing_credentials, github_workflows_commander_release_build_bundle, github_workflows_commander_release_notarize_staple, commander_script_build_and_run [EXTRACTED 0.90]
- **Release version resolution, dedup guard, asset packaging, and publish** — github_workflows_commander_release_resolve_version, github_workflows_commander_release_existing_release, github_workflows_commander_release_collect_assets, github_workflows_commander_release_generate_notes, github_workflows_commander_release_publish_release, commander_script_release_version [EXTRACTED 0.90]
...
Cohesion: 0.10
Nodes (22): build-all-branch.mjs, Doctor Commit Command, Lopu Internal All-branch Integration Workflow, Build-doctor Model Waterfall Loader, Rebuild Job, Union Build Check, ai-merge-paused Stop Signal, AI-powered PR Conflict Resolution (+14 more)
...
Cohesion: 0.30
Nodes (12): Commander build_and_run.sh, Commander release-version.mjs, Build Commander bundle, Collect release assets, Stop if this release already exists, Generate release notes, Import Developer ID and notarization credentials, Notarize and staple Commander bundle (+4 more)
...
- **Why does `selfTest()` connect `selfTest` to `detect job`, `promote-features-to-main.mjs`, `runPromotion`, `failureDetail`?**
...
- **Why does `failureDetail()` connect `failureDetail` to `detect job`, `selfTest`, `promote-features-to-main.mjs`, `runPromotion`?**
  _High betweenness centrality (0.001) - this node is a cross-community bridge._
