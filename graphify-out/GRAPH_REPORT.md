...
- `Deterministic merge verify step` --references--> `graphify-out/graph.json`  [EXTRACTED]
  .github/workflows/resolve-pr-conflicts.yml → graphify-out/graph.json
- `Post-merge Graphify refresh step` --references--> `graphify-out/graph.json`  [EXTRACTED]
  .github/workflows/resolve-pr-conflicts.yml → graphify-out/graph.json
...
- **Detector → control-plane handoff → resolve pipeline** — github_workflows_resolve_pr_conflicts_detect, github_workflows_resolve_pr_conflicts_handoff, github_workflows_resolve_pr_conflicts_resolve, github_workflows_resolve_pr_conflicts_control_plane_identity [EXTRACTED 0.90]
- **Trusted post-model verification and publication chain** — github_workflows_resolve_pr_conflicts_verify_step, github_workflows_resolve_pr_conflicts_scope_invariant, github_workflows_resolve_pr_conflicts_lockfile_regen, github_workflows_resolve_pr_conflicts_secret_scan, github_workflows_resolve_pr_conflicts_graphify_refresh [EXTRACTED 0.85]
- **AI provider credential selection across resolve and graph refresh** — github_workflows_resolve_pr_conflicts_anthropic_api_key, github_workflows_resolve_pr_conflicts_openai_api_key, github_workflows_resolve_pr_conflicts_claude_code_action, github_workflows_resolve_pr_conflicts_codex_action, github_workflows_resolve_pr_conflicts_graphify_refresh [EXTRACTED 0.80]
...
Cohesion: 0.06
Nodes (37): build-all-branch.mjs, Doctor Commit Command, Lopu Internal All-branch Integration Workflow, Build-doctor Model Waterfall Loader, Rebuild Job, Union Build Check, ai-merge-paused Stop Label, All-branch Doctor (+29 more)
...
- **Why does `selfTest()` connect `build-all-branch.mjs` to `verify-promotion-source-authority.sh`, `runPromotion`, `failureDetail`, `workflow-control-plane-contract.mjs`?**
