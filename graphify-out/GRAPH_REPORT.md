...
- `Lopu CodeQL All Branches Workflow` --conceptually_related_to--> `CodeQL Coverage Backfill`  [INFERRED]
  .github/workflows/codeql-analysis.yml → CHANGELOG.md
- `Post-Merge Graphify Refresh` --conceptually_related_to--> `Graphify Semantic Refresh`  [INFERRED]
...
- **Lopu Control Plane Flows** — changelog_lopu_pr_manager, github_workflows_resolve_pr_conflicts_workflow, github_workflows_resolve_pr_conflicts_control_plane, github_workflows_resolve_pr_conflicts_detector_handoff [EXTRACTED 0.90]
...
- **Post-Merge Graphify Semantics** — changelog_graphify_semantic_refresh, github_workflows_resolve_pr_conflicts_graphify_refresh, github_workflows_resolve_pr_conflicts_graphify_cli [EXTRACTED 0.86]
...
Cohesion: 0.11
Nodes (20): CodeQL Coverage Backfill, CodeQL PR Handoff, Conflict Resolution, Graphify Semantic Refresh, Lopu PR Manager, Promotion Recovery, actions/checkout, Analyze Job (+12 more)
...
### Community 11 - "`github-actions` — the CI control plane"
Cohesion: 0.12
...
### Community 25 - "electron-pr-release-contract.mjs"
Cohesion: 0.33
...
## Knowledge Gaps
- **149 isolated node(s):** `BASE_BRANCHES`, `MERGE_CONFIG`, `CAPACITY_PATTERNS`, `CREDENTIAL_PATTERNS`, `TRUSTED_ASSOCIATIONS` (+144 more)
...
- **Why does `selfTest()` connect `selfTest` to `promote-features-to-main.mjs`, `runPromotion`, `verify-promotion-source-authority.sh`, `failureDetail`?**
...
- **Why does `failureDetail()` connect `failureDetail` to `selfTest`, `promote-features-to-main.mjs`, `runPromotion`, `verify-promotion-source-authority.sh`?**
  _High betweenness centrality (0.001) - this node is a cross-community bridge._
