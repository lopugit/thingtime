...
## Summary
- 423 nodes · 779 edges · 42 communities (25 shown, 17 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 6 edges (avg confidence: 0.66)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f2927a4e`
...
3. `main()` - 15 edges
4. `buildMode()` - 14 edges
5. `githubRequest()` - 14 edges
6. `boundedInteger()` - 13 edges
7. `vercelRequest()` - 13 edges
8. `main()` - 12 edges
...
- `Lopu PR manager (AI conflict resolver)` --conceptually_related_to--> `Lopu internal feature promotion workflow`  [AMBIGUOUS]
  .github/workflows/resolve-pr-conflicts.yml → .github/workflows/promote-features-to-main.yml
- `Lopu PR manager (AI conflict resolver)` --references--> `pr-conflict-resolver.yml (superseded)`  [EXTRACTED]
  .github/workflows/resolve-pr-conflicts.yml → .github/workflows/pr-conflict-resolver.yml
- `Lopu internal feature promotion workflow` --references--> `Sync main into develop workflow`  [EXTRACTED]
...
- **develop → main promotion automation family** — github_workflows_promote_develop_to_main, github_workflows_promote_features_to_main, github_workflows_sync_main_into_develop, github_workflows_resolve_pr_conflicts [EXTRACTED 0.85]
- **Shared CI provider routing contract** — github_workflows_ci_provider_router, github_workflows_promote_develop_to_main_route, github_workflows_promote_features_to_main_route, github_workflows_control_plane_branch [EXTRACTED 0.80]
- **Detector-only handoff to protected control plane** — github_workflows_codeql_analysis_scope, github_workflows_resolve_pr_conflicts_detector_handoff, github_workflows_control_plane_branch, github_workflows_promote_features_to_main_lanes [INFERRED 0.70]
...
Cohesion: 0.07
Nodes (93): ACTIVE_STATES, assertCurrentPullRequest(), assertRepositoryDispatchSource(), assertTrustedPrincipal(), assertTrustedPullRequest(), assertTrustedPullRequestStack(), assertVercelConfiguration(), assertWildcardFallbackRuntimes() (+85 more)
...
Cohesion: 0.14
Nodes (18): CI provider router (reusable workflow), Lopu CodeQL all branches workflow, analyze: CodeQL language matrix, scope: Select one analysis owner, CodeQL PR handoff workflow, github-actions control-plane branch, pr-conflict-resolver.yml (superseded), Lopu internal develop promotion workflow (+10 more)
...
- `Lopu internal feature promotion workflow` → `Lopu PR manager (AI conflict resolver)`  [AMBIGUOUS]
  .github/workflows/resolve-pr-conflicts.yml · relation: conceptually_related_to

## Knowledge Gaps
- **135 isolated node(s):** `BASE_BRANCHES`, `MERGE_CONFIG`, `CAPACITY_PATTERNS`, `CREDENTIAL_PATTERNS`, `REQUIRED_CATEGORIES` (+130 more)
...
- **What is the exact relationship between `Lopu internal feature promotion workflow` and `Lopu PR manager (AI conflict resolver)`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `HttpError` connect `rebase-ownership-routing-contract.sh` to `deploy-develop-pr-preview.mjs`?**
