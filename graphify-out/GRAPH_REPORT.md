...
## Summary
- 599 nodes · 1295 edges · 46 communities (30 shown, 16 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 21 edges (avg confidence: 0.65)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `5fbaa44e`
...
- `Historical CodeQL coverage backfill entry` --references--> `Lopu CodeQL all branches workflow`  [INFERRED]
  CHANGELOG.md → .github/workflows/codeql-analysis.yml
- `Lopu as single public PR-management workflow entry` --references--> `Lopu PR manager workflow`  [INFERRED]
  CHANGELOG.md → .github/workflows/resolve-pr-conflicts.yml
- `Reverse and fan-out promotion lanes entry` --references--> `Lopu internal feature promotion workflow`  [INFERRED]
  CHANGELOG.md → .github/workflows/promote-features-to-main.yml
- `Deterministic delete-shaped promotion conflict entry` --references--> `Lopu rebase conflict round action`  [INFERRED]
...
- **Lopu PR manager is the single public entrypoint for every internal maintenance lane** — github_workflows_resolve_pr_conflicts, github_workflows_promote_develop_to_main, github_workflows_promote_features_to_main, github_workflows_codeql_analysis, github_actions_rebase_conflict_round_action, github_workflows_sync_main_into_develop [EXTRACTED 0.90]
- **Trusted rebase conflict round: bootstrap, prepare scratch, model resolve, session continuation** — github_actions_rebase_conflict_round_bootstrap, github_actions_rebase_conflict_round_prepare, github_actions_rebase_conflict_round_lopu, github_actions_rebase_conflict_round_claude_session_resume, github_scripts_rebase_stack_prepare_round, github_actions_lopu_agent [EXTRACTED 0.90]
- **Signed, notarized Commander macOS release flow** — github_workflows_commander_release_release, github_workflows_commander_release_import_credentials, github_workflows_commander_release_notarize, commander_script_build_and_run, commander_script_release_version [EXTRACTED 0.90]
...
Cohesion: 0.05
Nodes (54): Control-plane changelog, All-branch maintenance coalescing entry, Historical CodeQL coverage backfill entry, Commander macOS Developer ID signing/notarization entry, Deterministic delete-shaped promotion conflict entry, Lopu as single public PR-management workflow entry, Blobless clone promisor-object recovery entry, Reverse and fan-out promotion lanes entry (+46 more)
...
## Knowledge Gaps
- **149 isolated node(s):** `BASE_BRANCHES`, `MERGE_CONFIG`, `CAPACITY_PATTERNS`, `CREDENTIAL_PATTERNS`, `REQUIRED_CATEGORIES` (+144 more)
...
- **Why does `promote: promote merged develop PRs to main` connect `promotion-pr-changelog.mjs` to `runPromotion`?**
