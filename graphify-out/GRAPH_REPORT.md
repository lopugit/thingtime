...
## Summary
- 599 nodes · 1278 edges · 49 communities (32 shown, 17 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 16 edges (avg confidence: 0.63)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3caf74ff`
...
- `Lopu internal all-branch integration workflow` --conceptually_related_to--> `CI provider router workflow`  [AMBIGUOUS]
  .github/workflows/all-branch.yml → .github/workflows/ci-provider-router.yml
- `Lopu PR manager workflow` --references--> `PR conflict auto-resolver model waterfall setting`  [INFERRED]
  .github/workflows/resolve-pr-conflicts.yml → .github/workflows/all-branch.yml
- `Lopu PR manager workflow` --references--> `pr-conflict-resolver.yml (superseded)`  [EXTRACTED]
...
- **Lopu agent-backed repository automations** — github_actions_rebase_conflict_round_action, github_workflows_all_branch, github_workflows_rebase_pr_stacks, github_workflows_resolve_pr_conflicts, github_actions_lopu_agent_action [EXTRACTED 0.90]
- **All-branch build doctor repair loop** — github_workflows_all_branch_check1, github_workflows_all_branch_models, github_workflows_all_branch_doctor1, github_workflows_all_branch_commit1, github_workflows_all_branch_check2, github_workflows_all_branch_doctor2 [EXTRACTED 0.90]
- **Repo-less scratch rebase round flow** — github_actions_rebase_conflict_round_action_bootstrap, github_actions_rebase_conflict_round_action_prepare, github_actions_rebase_conflict_round_action_lopu, github_actions_rebase_conflict_round_action_verify, github_scripts_rebase_stack_prepare_round [EXTRACTED 0.90]
...
Cohesion: 0.15
Nodes (35): assertAllBranchWorkflowContract(), BASE_BRANCHES, buildMode(), checkMode(), countLeadingFailureMarkers(), doctorCommitMode(), doctorRecordMode(), git() (+27 more)
...
Cohesion: 0.09
Nodes (25): Lopu agent composite action, Lopu rebase conflict round action, attempt: mark credentialed AI attempt, bootstrap: copy trusted round code, lopu: resolve conflict set in scratch, prepare: validate conflicts, create scratch, verify: continue rebase deterministically, check2: re-check after round 1 (+17 more)
...
Cohesion: 0.29
Nodes (8): actions/checkout, Analyze Job, github/codeql-action/analyze, github/codeql-action/init, gh api, jq, Select One Analysis Owner Job, Lopu CodeQL All Branches Workflow
...
Cohesion: 0.67
Nodes (3): detect job (API-only conflict detection), handoff job (control-plane dispatch), no-ai-merge opt-out label

## Ambiguous Edges - Review These
- `CI provider router workflow` → `Lopu internal all-branch integration workflow`  [AMBIGUOUS]
  .github/workflows/all-branch.yml · relation: conceptually_related_to

## Knowledge Gaps
- **155 isolated node(s):** `BASE_BRANCHES`, `MERGE_CONFIG`, `CAPACITY_PATTERNS`, `CREDENTIAL_PATTERNS`, `REQUIRED_CATEGORIES` (+150 more)
...
- **What is the exact relationship between `CI provider router workflow` and `Lopu internal all-branch integration workflow`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `prepare: validate conflicts, create scratch` connect `refresh-promotion-graphify.sh` to `prepare-round.sh`?**
