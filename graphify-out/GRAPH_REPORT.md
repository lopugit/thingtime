...
- `Post-merge Graphify Refresh` --semantically_similar_to--> `Graphify Attestations`  [INFERRED] [semantically similar]
  .github/workflows/resolve-pr-conflicts.yml → CHANGELOG.md
...
- **Lopu Control Plane Entrypoints** — changelog_lopu_pr_manager, changelog_github_actions_branch, changelog_product_branch_listeners, _github_workflows_resolve_pr_conflicts_lopu_pr_manager_workflow, _github_workflows_resolve_pr_conflicts_workflow_call [EXTRACTED 0.92]
- **Post-merge Graphify Semantic Refresh** — changelog_graphify_attestations, changelog_openai_semantic_extraction, _github_workflows_resolve_pr_conflicts_graphify_refresh, _github_workflows_resolve_pr_conflicts_openai_api_key, _github_workflows_resolve_pr_conflicts_anthropic_api_key [EXTRACTED 0.90]
- **PR Automation Stop and Opt-out Labels** — changelog_ai_merge_paused, changelog_no_ai_merge, _github_workflows_resolve_pr_conflicts_ai_merge_paused, _github_workflows_resolve_pr_conflicts_no_ai_merge [EXTRACTED 0.88]
...
Cohesion: 0.12
Nodes (20): ANTHROPIC_API_KEY Secret, Detector Handoff Input, Post-merge Graphify Refresh, issue_comment Trigger, Lopu PR Manager Workflow, Maintenance Operation Input, no-ai-merge Label, OPENAI_API_KEY Secret (+12 more)
...
## Knowledge Gaps
- **147 isolated node(s):** `BASE_BRANCHES`, `MERGE_CONFIG`, `CAPACITY_PATTERNS`, `CREDENTIAL_PATTERNS`, `TRUSTED_ASSOCIATIONS` (+142 more)
...
- **Why does `selfTest()` connect `selfTest` to `runPromotion`, `promote-features-to-main.mjs`, `classify-claude-credential-failure.mjs`, `failureDetail`?**
...
- **Why does `failureDetail()` connect `failureDetail` to `selfTest`, `classify-claude-credential-failure.mjs`, `promote-features-to-main.mjs`, `runPromotion`?**
