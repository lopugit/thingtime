...
- [[_COMMUNITY_Lopu Internal Main Develop Synchronization Workflow|Lopu Internal Main Develop Synchronization Workflow]]
- [[_COMMUNITY_rebase-ownership-routing-contract.sh|rebase-ownership-routing-contract.sh]]
...
- `Post-Merge Graphify Refresh` --conceptually_related_to--> `Lopu Agent Composite Action`  [INFERRED]
...
- **CI Control Plane Files** — readme_github_workflows, readme_github_scripts, readme_github_actions, readme_vercel_json [EXTRACTED 1.00]
- **CodeQL Analysis Flow** — github_workflows_codeql_analysis_workflow, github_workflows_codeql_analysis_scope_job, github_workflows_codeql_analysis_analyze_job, github_workflows_codeql_analysis_codeql_init_action, github_workflows_codeql_analysis_codeql_analyze_action [EXTRACTED 1.00]
- **Lopu Control Plane Components** — readme_lopu_pr_manager, readme_lopu_agent_action, readme_codeql_analysis_workflow [EXTRACTED 0.90]
...
Cohesion: 0.15
Nodes (14): analyze job, actions/checkout v6.1.0, github/codeql-action/analyze v4, github/codeql-action/init v4, gh api, jq, scope job, Prefer the PR analysis for an open branch head step (+6 more)

### Community 15 - "Thingtime AI instructions"
Cohesion: 0.19
Nodes (14): actions/checkout@v4, Check develop exists Step, Checkout develop Step, ci-provider-router.yml, CONFLICT_RESOLVER_PAT Secret, find_sync_pr Function, GITHUB_TOKEN, Merge main into develop Step (+6 more)
...
Cohesion: 0.17
Nodes (12): AI_ALL.md, CI control plane, .github/scripts/deploy-develop-pr-preview.mjs, electron-pr-release.yml, .github/actions, github-actions branch, .github/scripts, .github/workflows (+4 more)
...
### Community 30 - "Lopu Internal Main Develop Synchronization Workflow"
Cohesion: 0.83
Nodes (3): assert_owner(), assert_stack(), rebase-ownership-routing-contract.sh script

## Knowledge Gaps
- **169 isolated node(s):** `BASE_BRANCHES`, `MERGE_CONFIG`, `CAPACITY_PATTERNS`, `CREDENTIAL_PATTERNS`, `TRUSTED_ASSOCIATIONS` (+164 more)
...
- **Why does `selfTest()` connect `build-all-branch.mjs` to `Build And Publish Signed PR Release Job`, `runPromotion`, `failureDetail`, `workflow-control-plane-contract.mjs`?**
