...
- `Lopu PR manager workflow` --conceptually_related_to--> `Lopu agent action`  [INFERRED]
  .github/workflows/resolve-pr-conflicts.yml → .github/actions/lopu-agent/action.yml
- `Lopu CodeQL all branches workflow` --conceptually_related_to--> `github-actions control-plane identity`  [INFERRED]
...
- **Bounded trusted-copy rebase conflict round** — github_actions_rebase_conflict_round_action_bootstrap, github_actions_rebase_conflict_round_action_prepare, github_actions_rebase_conflict_round_action_attempt, github_actions_rebase_conflict_round_action_lopu, github_actions_rebase_conflict_round_action_verify, github_scripts_rebase_stack_prepare_round, github_actions_lopu_agent_action [EXTRACTED 0.90]
- **develop → main promotion automation family** — github_workflows_promote_develop_to_main, github_workflows_promote_features_to_main, github_workflows_sync_main_into_develop, github_workflows_resolve_pr_conflicts, github_workflows_ci_provider_router [EXTRACTED 0.85]
- **github-actions control-plane handoff pattern** — github_workflows_resolve_pr_conflicts, github_workflows_resolve_pr_conflicts_control_plane_identity, github_workflows_resolve_pr_conflicts_detector_handoff, github_workflows_codeql_analysis, github_workflows_codeql_pr_handoff, remix_scripts_workflow_caller_contract [INFERRED 0.75]
...
Cohesion: 0.07
Nodes (30): Lopu agent action, Lopu rebase conflict round action, Mark the credentialed AI attempt, Copy trusted round code outside the model workspace, hash_trusted_tree, Resolve rebase conflict set with Lopu in scratch, Validate conflicts and create repo-less scratch, sha256_file (+22 more)
...
## Knowledge Gaps
- **143 isolated node(s):** `BASE_BRANCHES`, `MERGE_CONFIG`, `CAPACITY_PATTERNS`, `CREDENTIAL_PATTERNS`, `REQUIRED_CATEGORIES` (+138 more)
...
- **Why does `Promote merged develop PRs job` connect `promotion-pr-changelog.mjs` to `promote-features-to-main.mjs`?**
