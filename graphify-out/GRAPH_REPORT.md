...
## Summary
- 532 nodes · 1151 edges · 44 communities (28 shown, 16 thin omitted)
- Extraction: 98% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 17 edges (avg confidence: 0.61)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ba048940`
...
- `Lopu PR manager workflow` --conceptually_related_to--> `Lopu agent action`  [AMBIGUOUS]
  .github/workflows/resolve-pr-conflicts.yml → .github/actions/lopu-agent/action.yml
- `Lopu rebase conflict round (composite action)` --conceptually_related_to--> `Lopu PR manager workflow`  [INFERRED]
  .github/actions/rebase-conflict-round/action.yml → .github/workflows/resolve-pr-conflicts.yml
- `Lopu internal develop promotion workflow` --conceptually_related_to--> `Lopu PR manager workflow`  [INFERRED]
  .github/workflows/promote-develop-to-main.yml → .github/workflows/resolve-pr-conflicts.yml
...
- **develop → main promotion automation family** — github_workflows_promote_develop_to_main, github_workflows_promote_features_to_main, github_workflows_sync_main_into_develop, github_workflows_resolve_pr_conflicts, github_scripts_promotion_pr_changelog, github_scripts_promote_features_to_main [EXTRACTED 0.85]
- **Thingtime CI Control provider routing across workflows** — github_workflows_ci_provider_router, github_workflows_promote_develop_to_main_route, github_workflows_promote_features_to_main_route, github_workflows_resolve_pr_conflicts, github_workflows_resolve_pr_conflicts_control_plane [INFERRED 0.75]
...
Cohesion: 0.11
Nodes (20): Lopu agent action, Lopu rebase conflict round (composite action), Copy trusted round code outside the model workspace, Resolve this rebase conflict set with Lopu in scratch, Validate conflicts and create repo-less scratch, Repo-less scratch model isolation, Verify step (complete/retry_needed/conflict outputs), CI provider router workflow (+12 more)
...
Cohesion: 0.67
Nodes (4): Lopu CodeQL all branches workflow, CodeQL analyze job, CodeQL scope job (select one analysis owner), CodeQL PR handoff workflow
...
- `Lopu agent action` → `Lopu PR manager workflow`  [AMBIGUOUS]
...
- **What is the exact relationship between `Lopu agent action` and `Lopu PR manager workflow`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `Promote merged develop PRs job` connect `refresh-promotion-graphify.sh` to `build-all-branch.mjs`?**
