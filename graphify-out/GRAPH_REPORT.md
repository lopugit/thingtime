...
9. `queueTrustedPromotionWorker()` - 11 edges
10. `selfTest()` - 11 edges

## Surprising Connections (you probably didn't know these)
- `Lopu Principal Repository Manager` --references--> `Develop PR Preview Deployment Script`  [EXTRACTED]
  README.md → .github/scripts/deploy-develop-pr-preview.mjs
- `github-actions CI Control Plane` --references--> `Workflow Control Plane Contract`  [EXTRACTED]
  README.md → .github/scripts/workflow-control-plane-contract.mjs
- `Thin Product Branch Listeners` --references--> `Workflow Caller Contract`  [EXTRACTED]
...
- `Post-merge Graphify Refresh` --conceptually_related_to--> `OpenAI Semantic Extraction`  [INFERRED]
...
- **Lopu Repository Management Lanes** — changelog_lopu_pr_manager, changelog_all_branch_doctor, changelog_promotion_recovery, changelog_rebase_stack_recovery, changelog_codeql_triage, readme_lopu_principal_repository_manager [EXTRACTED 0.92]
...
- **Graphify Semantic Refresh Provider Flow** — changelog_graphify_attestations, changelog_openai_semantic_extraction, _github_workflows_resolve_pr_conflicts_graphify_refresh, _github_workflows_resolve_pr_conflicts_graphify_cli, readme_lopu_principal_repository_manager [INFERRED 0.82]
...
Cohesion: 0.09
Nodes (23): actions/checkout, actions/setup-node, Build All Branch Script, Lopu Agent Action, Lopu Internal All-branch Integration Workflow, Build-doctor Model Waterfall, pnpm/action-setup, PR Conflict Resolution (+15 more)
...
Cohesion: 0.40
Nodes (5): github-actions CI Control Plane, Thin Product Branch Listeners, Vercel Git Deployment Kill Switch, Workflow Caller Contract, Workflow Control Plane Contract
...
## Knowledge Gaps
- **137 isolated node(s):** `CAPACITY_PATTERNS`, `CREDENTIAL_PATTERNS`, `here`, `workflow`, `CFG` (+132 more)
...
- **Why does `selfTest()` connect `promotion-pr-changelog.mjs` to `runPromotion`, `selfTest`, `Publish or reconcile develop S3 preview job`, `failureDetail`?**
  _High betweenness centrality (0.005) - this node is a cross-community bridge._
- **Why does `runPromotion()` connect `runPromotion` to `promotion-pr-changelog.mjs`, `selfTest`, `failureDetail`?**
  _High betweenness centrality (0.002) - this node is a cross-community bridge._
- **Why does `failureDetail()` connect `failureDetail` to `promotion-pr-changelog.mjs`, `selfTest`, `runPromotion`, `Publish or reconcile develop S3 preview job`?**
  _High betweenness centrality (0.001) - this node is a cross-community bridge._
- **What connects `CAPACITY_PATTERNS`, `CREDENTIAL_PATTERNS`, `here` to the rest of the system?**
