...
## Corpus Check
- 21 files · ~126,114 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 553 nodes · 1195 edges · 47 communities (33 shown, 14 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 10 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b183de6a`
...
- **Control Plane PR Management Concepts** — changelog_github_actions_branch, changelog_product_branch_listeners, changelog_lopu_pr_management, changelog_conflict_resolution, changelog_promotion_lanes, github_workflows_resolve_pr_conflicts_workflow [INFERRED 0.82]
...
## Knowledge Gaps
- **150 isolated node(s):** `workflow`, `rebaseWorkflow`, `allBranchWorkflow`, `action`, `worker` (+145 more)
...
- **Why does `selfTest()` connect `selfTest` to `prepare-round.sh`, `promote-features-to-main.mjs`, `runPromotion`, `promotion-worker.sh`, `CodeQL All Branches Workflow`?**
  _High betweenness centrality (0.003) - this node is a cross-community bridge._
- **Why does `runPromotion()` connect `CodeQL All Branches Workflow` to `selfTest`, `prepare-round.sh`, `promote-features-to-main.mjs`, `runPromotion`, `promotion-worker.sh`?**
