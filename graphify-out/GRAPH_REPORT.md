...
6. `main()` - 20 edges
7. `Lopu PR Manager Workflow` - 20 edges
8. `orphanedMergeHydrationIntegrationTest()` - 19 edges
9. `main()` - 15 edges
10. `repoFlag()` - 15 edges

## Surprising Connections (you probably didn't know these)
- `Lopu rebase engine workflow` --semantically_similar_to--> `Reusable stack rebase engine`  [INFERRED] [semantically similar]
...
- `Lopu PR Manager Workflow` --references--> `ai-merge-paused stop label`  [EXTRACTED]
...
- **Lopu control-plane workflows** — changelog_lopu_pr_manager, github_workflows_resolve_pr_conflicts_lopu_pr_manager, github_workflows_rebase_pr_stacks_lopu_rebase_engine [EXTRACTED 0.88]
- **Workflow Event Triggers** — github_workflows_resolve_pr_conflicts_workflow_call, github_workflows_resolve_pr_conflicts_push, github_workflows_resolve_pr_conflicts_pull_request_target, github_workflows_resolve_pr_conflicts_issue_comment, github_workflows_resolve_pr_conflicts_pull_request_review_comment, github_workflows_resolve_pr_conflicts_check_run, github_workflows_resolve_pr_conflicts_schedule [EXTRACTED 1.00]
- **Post-Merge Graphify Refresh Flow** — github_workflows_resolve_pr_conflicts_graphify_refresh, github_workflows_resolve_pr_conflicts_graphify_extract, github_workflows_resolve_pr_conflicts_graphify_cluster_only, github_workflows_resolve_pr_conflicts_graphify_update, github_workflows_resolve_pr_conflicts_graphify_out [EXTRACTED 1.00]
- **Automation Control Labels** — github_workflows_resolve_pr_conflicts_no_ai_merge, github_workflows_resolve_pr_conflicts_no_ai_rebase, github_workflows_resolve_pr_conflicts_ai_merge_paused [EXTRACTED 1.00]
...
Cohesion: 0.06
Nodes (37): ai-merge-paused stop label, Evidence-backed CodeQL triage, Control-plane changelog, Post-merge Graphify refresh, Lopu PR manager, Promotion lanes, Reusable stack rebase engine, ci-provider-router reusable workflow (+29 more)
...
Cohesion: 0.15
Nodes (14): classify-claude-credential-failure.mjs, Run Lopu with Fallback Claude Credential, Run Lopu with Primary Claude Credential, Run Lopu with Codex, Lopu Agent Composite Action, Validate the Lopu Backend, prepare-round.sh, Lopu Rebase Conflict Round Composite Action (+6 more)
...
### Community 30 - "Thingtime Electron PR Release Workflow"
Cohesion: 0.67
Nodes (3): no-ai-rebase label, REBASE_OWNER_JQ, STACK_MEMBER_JQ

## Knowledge Gaps
- **167 isolated node(s):** `workflow`, `rebaseWorkflow`, `allBranchWorkflow`, `developPromotionWorkflow`, `featurePromotionWorkflow` (+162 more)
...
- **Why does `selfTest()` connect `runPromotion` to `promotion-worker-contract.sh`, `failureDetail`, `workflow-control-plane-contract.mjs`, `start.sh`?**
