...
## Summary
- 599 nodes · 1291 edges · 52 communities (36 shown, 16 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 22 edges (avg confidence: 0.68)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `00b7ddfa`
...
- `All-branch maintenance coalescing entry` --references--> `Lopu PR manager workflow`  [INFERRED]
  CHANGELOG.md → .github/workflows/resolve-pr-conflicts.yml
- `Blobless partial-clone hydration recovery entry` --references--> `Lopu PR manager workflow`  [INFERRED]
  CHANGELOG.md → .github/workflows/resolve-pr-conflicts.yml
- `Historical CodeQL coverage backfill entry` --references--> `Lopu PR manager workflow`  [INFERRED]
  CHANGELOG.md → .github/workflows/resolve-pr-conflicts.yml
- `Reverse and multi-target promotion lane entries` --references--> `Lopu internal feature promotion workflow`  [INFERRED]
  CHANGELOG.md → .github/workflows/promote-features-to-main.yml
- `Commander App Release workflow` --conceptually_related_to--> `Thin control-plane bare-tree contract`  [AMBIGUOUS]
  .github/workflows/commander-release.yml → CHANGELOG.md
...
- **Lopu single public PR-management control plane** — github_workflows_resolve_pr_conflicts, github_workflows_promote_develop_to_main, github_workflows_promote_features_to_main, github_actions_rebase_conflict_round_action, github_workflows_codeql_analysis, github_actions_lopu_agent_action [EXTRACTED 0.90]
- **Trusted-copy, scratch-prepare, model-round isolation flow** — github_actions_rebase_conflict_round_bootstrap, github_actions_rebase_conflict_round_prepare, github_actions_rebase_conflict_round_lopu_step, github_actions_rebase_conflict_round_continue_session, github_scripts_rebase_stack_prepare_round [EXTRACTED 0.85]
- **Signed, notarized Commander release flow** — github_workflows_commander_release_release, github_workflows_commander_release_import_credentials, github_workflows_commander_release_notarize, commander_script_build_and_run, changelog_commander_signing_entry [EXTRACTED 0.80]
...
Cohesion: 0.23
Nodes (12): Durable ai-merge-paused stop label, CI provider router workflow, pr-conflict-resolver.yml (superseded), Lopu internal develop promotion workflow, promotion-pr job, Lopu internal feature promotion workflow, lanes job: fan out CI promotion lanes, promote job (+4 more)
...
Cohesion: 0.25
Nodes (11): Single protected Lopu agent backend entry, Lopu agent action, Lopu rebase conflict round action, Mark the credentialed AI attempt, Copy trusted round code outside the model workspace, hash_trusted_tree, Resolve rebase conflict set with Lopu in scratch, Validate conflicts and create repo-less scratch (+3 more)

### Community 18 - "Publish or reconcile develop S3 preview job"
Cohesion: 0.22
Nodes (9): Control-plane changelog, All-branch maintenance coalescing entry, Commander macOS Developer ID signing entry, Blobless partial-clone hydration recovery entry, Reverse and multi-target promotion lane entries, Thin control-plane bare-tree contract, Commander App Release workflow, Electron PR release workflow (+1 more)
...
Cohesion: 0.50
Nodes (5): Historical CodeQL coverage backfill entry, Lopu CodeQL all branches workflow, analyze job (matrix language), scope job: select one analysis owner, CodeQL pull_request_target metadata handoff

### Community 30 - "STACK_MEMBER_JQ"
Cohesion: 0.50
Nodes (5): build_and_run.sh, release-version.mjs, Import Developer ID and notarization credentials, Notarize and staple Commander bundle, release job: build and publish Commander

### Community 31 - "Lopu Build Doctor Round 1"
Cohesion: 0.50
Nodes (4): assertPrReleaseContract(), count(), here, workflow

### Community 32 - "Web CI Workflow"
Cohesion: 0.83
Nodes (3): assert_owner(), assert_stack(), rebase-ownership-routing-contract.sh script

### Community 33 - "Safe Main into Develop Sync PR"
Cohesion: 1.00
Nodes (3): classify_output, Continue the exact Claude session until it finishes, read_classification

### Community 34 - "Promotion PR Changelog Script"
Cohesion: 0.67
Nodes (3): gh workflow run codeql-analysis.yml, Lopu Queues Unprivileged PR Scan Job, Lopu CodeQL PR Handoff Workflow

### Community 35 - "Route CI Compute Provider Workflow"
Cohesion: 0.67
Nodes (3): no-ai-rebase label, REBASE_OWNER_JQ, STACK_MEMBER_JQ

## Ambiguous Edges - Review These
- `Thin control-plane bare-tree contract` → `Commander App Release workflow`  [AMBIGUOUS]
  .github/workflows/commander-release.yml · relation: conceptually_related_to

## Knowledge Gaps
- **153 isolated node(s):** `BASE_BRANCHES`, `MERGE_CONFIG`, `CAPACITY_PATTERNS`, `CREDENTIAL_PATTERNS`, `REQUIRED_CATEGORIES` (+148 more)
...
- **What is the exact relationship between `Thin control-plane bare-tree contract` and `Commander App Release workflow`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `Lopu internal feature promotion workflow` connect `promotion-worker-routing-contract.mjs` to `Publish or reconcile develop S3 preview job`?**
  _High betweenness centrality (0.077) - this node is a cross-community bridge._
- **Why does `promote job` connect `promotion-worker-routing-contract.mjs` to `promote-features-to-main.mjs`?**
  _High betweenness centrality (0.075) - this node is a cross-community bridge._
- **Why does `Lopu PR manager workflow` connect `promotion-worker-routing-contract.mjs` to `Run Lopu with Primary Claude Credential`, `Publish or reconcile develop S3 preview job`, `Lopu Queues Unprivileged PR Scan Job`?**
