...
- `Lopu PR manager (resolve-pr-conflicts)` --references--> `pr-conflict-resolver.yml (superseded)`  [EXTRACTED]
  .github/workflows/resolve-pr-conflicts.yml → .github/workflows/pr-conflict-resolver.yml
- `Lopu CodeQL all branches workflow` --conceptually_related_to--> `github-actions control-plane branch identity`  [INFERRED]
  .github/workflows/codeql-analysis.yml → .github/workflows/resolve-pr-conflicts.yml
- `Lopu PR manager (resolve-pr-conflicts)` --references--> `no-ai-rebase promotion label`  [EXTRACTED]
  .github/workflows/resolve-pr-conflicts.yml → .github/workflows/promote-develop-to-main.yml
...
- **develop → main promotion automation flow** — github_workflows_promote_develop_to_main, github_workflows_promote_features_to_main, github_workflows_sync_main_into_develop, github_scripts_promotion_pr_changelog, github_scripts_promote_features_to_main [EXTRACTED 0.85]
- **Shared CI provider routing contract** — github_workflows_ci_provider_router, github_workflows_promote_develop_to_main_route, github_workflows_promote_features_to_main_route, github_workflows_resolve_pr_conflicts [EXTRACTED 0.80]
- **github-actions control-plane listener pattern** — github_workflows_resolve_pr_conflicts, github_workflows_resolve_pr_conflicts_control_plane, remix_scripts_workflow_caller_contract, github_workflows_codeql_analysis, github_workflows_codeql_pr_handoff [INFERRED 0.70]
...
Cohesion: 0.16
Nodes (19): CI provider router workflow, Lopu CodeQL all branches workflow, analyze job (CodeQL matrix), scope job (select one analysis owner), codeql-pr-handoff workflow, pr-conflict-resolver.yml (superseded), Lopu internal develop promotion workflow, no-ai-rebase promotion label (+11 more)
...
## Knowledge Gaps
- **150 isolated node(s):** `BASE_BRANCHES`, `MERGE_CONFIG`, `CAPACITY_PATTERNS`, `CREDENTIAL_PATTERNS`, `REQUIRED_CATEGORIES` (+145 more)
...
- **Why does `promote job` connect `codeql-open-pr-backfill.mjs` to `refresh-promotion-graphify.sh`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
- **Why does `Lopu internal develop promotion workflow` connect `codeql-open-pr-backfill.mjs` to `promotion-pr-changelog.mjs`?**
