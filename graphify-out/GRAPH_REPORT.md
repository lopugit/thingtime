...
## Summary
- 483 nodes · 1041 edges · 43 communities (25 shown, 18 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 16 edges (avg confidence: 0.6)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9352ccdd`
...
- `All-branch rebuild job` --conceptually_related_to--> `Lopu rebase engine workflow`  [AMBIGUOUS]
  .github/workflows/all-branch.yml → .github/workflows/rebase-pr-stacks.yml
...
- **Lopu PR automation fleet sharing the agent action and credentials** — github_workflows_resolve_pr_conflicts, github_workflows_rebase_pr_stacks, github_workflows_all_branch, github_actions_rebase_conflict_round_action, github_actions_lopu_agent_action [EXTRACTED 0.85]
- **develop → main promotion flow (omnibus + per-feature lanes)** — github_workflows_promote_develop_to_main, github_workflows_promote_features_to_main, github_scripts_promotion_pr_changelog, github_scripts_promote_features_to_main, github_workflows_ci_provider_router [EXTRACTED 0.80]
- **Bounded rebase conflict round: prepare, model resolve, verify** — github_actions_rebase_conflict_round_action_bootstrap, github_actions_rebase_conflict_round_action_prepare, github_actions_rebase_conflict_round_action_lopu, github_actions_rebase_conflict_round_action_verify, github_scripts_rebase_stack_prepare_round [EXTRACTED 0.90]
...
Cohesion: 0.16
Nodes (16): Lopu agent composite action, Lopu rebase conflict round action, Copy trusted round code step, Resolve conflict set with Lopu step, Validate conflicts and create scratch step, Verify and continue rebase step, Lopu internal all-branch integration workflow, All-branch rebuild job (+8 more)
...
Cohesion: 0.50
Nodes (4): Lopu CodeQL all branches workflow, CodeQL analyze job, Select one analysis owner job, CodeQL PR handoff workflow

## Ambiguous Edges - Review These
- `All-branch rebuild job` → `Lopu rebase engine workflow`  [AMBIGUOUS]
...
- **What is the exact relationship between `All-branch rebuild job` and `Lopu rebase engine workflow`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `promote job` connect `refresh-promotion-graphify.sh` to `failureDetail`?**
