...
**Status:** 🌱 Idea bank · seeded 2026-07-08 · selected eggs are present in the
current repository snapshot (marked ✅); reverify the exact head and browser
behavior before calling them shipped.
...
- ✅ **Konami code → party mode.** `↑↑↓↓←→←→ B A` anywhere fires a confetti
  ...
  and keyboard people hunt for; none existed before. _(app-wide keydown listener;
  reuses `burstConfetti` + `useLopu`.)_
- ✅ **Unicorn logo gallop.** A rapid 7-click streak on the nav 🦄 logo makes it
  ...
  double-clicks never trigger it. _(`Nav.tsx` logo + mobile twin.)_
- ✅ **Secret Commander words.** Type a magic word into the "Imagine.." Commander
  ...
  mode. Rides the existing `executeCommand` branch. _(`CommanderV2.tsx`.)_
- ✅ **`window.lopu` console API + rainbow banner.** On boot, print a `%c`
  ...
  open the console get a grin and a real tool. _(`EasterEggs.tsx` boot effect +
  `eggs/consoleEgg.ts`; `RAINBOW_PALETTE`.)_
- ✅ **The footer wizard casts spells.** The 🌈🦄🧙 footer row is decorative
  ...
  a Lopu toast with a rotating incantation ("✨ Levio-thing-sa"). _(`Footer.tsx`
  icon row.)_
- 🔮 **Lopu's 11:11 make-a-wish musing.** The musing endpoint already computes the
  ...
  test==live because it keys off the same tz param. _(`_musing.tsx` +
  `fallbacks.ts`.)_
- 🔮 **Ultra-rare Icon rolls + secret names.** `Icon.tsx` already coin-flips
  ...
  Dec 25 → 🎄, deterministic-by-date). _(`Icon.tsx`.)_
- 🔮 **API personality: `/api/v1/teapot` + a Lopu 404.** Replace the dispatcher's
  ...
  `-docs` twin). Rewards devs exploring the self-describing API. _(`[...].ts` 404;
  `apiDocs.ts` registration.)_

## ✨ Features (growth / stickiness)

- ✅ **Post permalinks + copy-link share** _(M — shipped via PR #141)_. Posts
...
- **Shareable algorithms — "try my feed brain 🧠"** _(M)_. Algorithms are already
  ...
  `?apply=<id>` deep links already are). _(`AlgorithmMenu`, `AlgorithmManager`,
  `algorithms/_`, `Feed.tsx` `?algorithm` param.)\*
- **Public theme gallery** _(M)_. `themes/shared` + `?apply=<shareId>` already
  ...
  feature, made a page. _(`ThemeStudio` preview extract, `themes/shared` list mode,
  new route.)_
- **"Wear my theme" on profiles** _(S)_. `user.activeThemeId` already syncs and
  ...
  storefronts. _(`ProfilePage`, `users/profile`, `useTtTheme`.)_
- **Clickable tags → public tag feeds** _(M)_. Tag chips are inert; the composer
  ...
  pattern). Shareable, guest-visible topic hubs. _(`PostCard`, `FeedFilters`,
  `Feed.tsx`, `things.ts` feed filter.)_
- **Circles become real** _(L)_. Posts target `friends`/`family` circles and
  ...
  graph. _(`ProfilePage`, `feedTypes` `CIRCLE_META`, new `users/circles`,
  `things.ts` visibility.)_
- **Waitlist welcome fortune** _(S)_. Joining already fires confetti but the API
  ...
  referral loop ("You're #412 — every friend moves you up 🌈"). _(`_waitlist.tsx`,
  `Landing.tsx`, `fallbacks.ts`.)_

## 🎨 Design

- **Algorithms grow up: 🥚 → 🐣 → 🐥 → 🧠** _(M)_. The pulsing "Learning as you
...
wings 🐥 → thinking 🧠`, with one Lopu toast per milestone ("Your algorithm just
  ...
  people a reason to come back and raise their feed. _(`AlgorithmMenu.tsx` dot
  title ~L341, `AlgorithmManager.tsx` label ~L248, `useFeedEngagement.ts`
  thresholds.)_ **See the mockup in `docs/design/thingtime-algorithm-growth/`.**
...
- ✅ **`Cmd+K` opens the Commander anywhere** _(S — shipped via PR #130)_. A window
  ...
  the palette yields inside a `.codex-editor` block. _(`commanderShortcut.ts`,
  `CommanderV2.tsx`.)_
- ✅ **Commander `>` command registry** _(M — shipped via PR #130)_. A `>` prefix
  ...
  _(`commanderCommands.ts`, `CommanderV2.tsx` `executeCommand`.)_
...
- **"Run it ▶" live API playground on `/docs/api`** _(M)_. Every endpoint already
  ...
  _(`apiDocs.ts`, `/docs/api` component, `[...].ts` `-docs` payloads.)_
- **DevKit request log + copy-as-curl** _(M)_. DevKit is already a draggable panel
  ...
  the same generator `apiDocs.ts` uses. _(`DevKit.tsx`, `useApi.tsx`.)_
- **Shake-to-open DevKit in the iOS app** _(M)_. `DevKit` already detects the
  ...
  toggles on, with a "You shook it! 🛠️" Lopu toast. _(`DevKit.tsx`,
  `NativeBridgeHost`, `iOS/` webview container.)_
...
the menu, not the commitment. Before any decorative feedback, celebration,
surprise, or play pattern is promoted, apply
[42 — Joyful delight and play agency](./42-joyful-delight-and-play-agency.md) as
the execution and non-coercion contract. Cross-feature sequencing, privacy-safe
outcome measurement, accessibility/reliability gates, and sustainable adoption
belong to [22 — Trustworthy adoption loop](./22-trustworthy-adoption-loop.md)
and its [phased roadmap](../../PLAN/trustworthy-adoption-roadmap.md).
