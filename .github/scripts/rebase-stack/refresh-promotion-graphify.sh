...
if [ -s "$RUNNER_TEMP/lopu-claude-credential-token" ]; then
  CLAUDE_CODE_OAUTH_TOKEN="$(<"$RUNNER_TEMP/lopu-claude-credential-token")"
else
...
node "$GITHUB_ACTION_PATH/../../scripts/lopu-credential-vault.mjs" needles "$needles"
