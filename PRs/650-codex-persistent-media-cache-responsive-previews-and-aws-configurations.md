...
## Authorized main promotion — 2026-09-05

The user explicitly requested merging this work into Thingtime main. The
`codex/persistent-media-cache-main` branch starts from current production main
and applies only the owned changes from PR #650, excluding unrelated develop
history. README, TESTING, changelog and capability-test conflicts preserve the
current main content and add only the media/configuration sections. Application
source changes applied cleanly. The focused tests and production build are
rerun on this main-based branch before merge; deployment receipts belong to the
main promotion PR and GitHub/Vercel records rather than a guessed future SHA.
