...
- Apple's authenticated status workflow `34439028802` confirms the existing 1.0 (30) build is VALID / IN_BETA_TESTING (external: READY_FOR_BETA_SUBMISSION). That build predates this feature's new native recovery changes, so this branch now reserves build 31; no upload of 31 is claimed. Release Xcode 26.6 (17F113), using iPhoneOS/watchOS 26.5 SDKs, successfully compiled the unsigned iPhone/Watch app and Icon Composer assets at the preceding source-equivalent build number. The app's embedded URL is `https://thingtime.com`.
