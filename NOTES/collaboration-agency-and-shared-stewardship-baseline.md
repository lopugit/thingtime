...
| Messenger has relational membership and bounded roles. | `chat-member` and `community-member` are separate Things. `ChatRole` is `owner | admin | member`; membership is checked on Messenger reads and writes. These roles govern chat/community operations, not arbitrary Thing edits. | High for the inspected code. Recheck after Messenger or community authorization changes. |
