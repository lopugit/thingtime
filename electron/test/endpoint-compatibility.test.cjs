...
test('capability contracts use compatible major versions and reject a missing or breaking feature', async () => {
	...
	assert.equal(DESKTOP_REQUIRED_CAPABILITIES['api.devices'], '^1.7.0');
	assert.equal(supportsVersion('1.6.0', DESKTOP_REQUIRED_CAPABILITIES['api.devices']), false);

	const server = http.createServer((request, response) => {
		response.setHeader('content-type', 'application/json');
		response.end(JSON.stringify({ ok: true, schemaVersion: 1, features: { 'api.devices': '1.7.0', 'api.posts': '1.0.0' } }));
	});
