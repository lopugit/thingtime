...
// api/utils/actions/packs/thingmon/data/species.json. Re-running the script
...
const DATA_FILE = path.join(remixRoot, 'app', 'api', 'utils', 'actions', 'packs', 'thingmon', 'data', 'species.json');
...
const json = JSON.stringify(catalogue, null, '\t') + '\n';
if (existsSync(DATA_FILE) && readFileSync(DATA_FILE, 'utf8') === json) console.log(`species.json unchanged (${catalogue.length} species)`);
else {
	writeFileSync(DATA_FILE, json);
	console.log(`species.json written (${catalogue.length} species)`);
}
