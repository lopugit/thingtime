...
async function main() {
	...
	assert.ok(
		url.protocol === 'http:' &&
			...
			!url.search &&
			!url.hash,
		'Use an explicit loopback QA origin'
	);
	...
	let team: any;
	try {
		team = JSON.parse(await readFile(process.env.TT_BUILDER_QA_TEAM_SESSION!, 'utf8'));
	} catch {
	...
	const rootId = 'qa-editable-builder-root',
		pageId = 'qa-editable-builder-page';
	...
	if (previous?.values.archived) {
		await call(owner, '/api/v1/builder/workspaces', {
			operation: 'archive',
			rootId,
			id: previous.id,
			archived: false,
	...
	await call(owner, '/api/v1/builder/workspaces', {
		...
		kind: 'member',
		values: { username: team.user.username, role: 'Employee' },
	...
	const foreign = await request(team, '/api/v1/actions/run', {
	...
	assert.equal(copied.copied, workspaceAppComposition({ namespace: 'qa-editable-app', rootId, pagePath: '/p/' + pageId }).definitions.length + 1);
	...
	const host = {
		...
		request: (step: any) =>
			call(
				team,
				step.path + (Object.keys(step.query).length ? '?' + new URLSearchParams(step.query) : ''),
				step.method === 'GET' ? undefined : step.body
			),
		prepare
	};
	async function read() {
	...
	assert.equal(
		(
			await request(team, '/api/v1/builder/workspaces', {
				operation: 'save',
				rootId,
				id: 'qa-builder-customer',
				kind: 'customer',
				values: { firstName: 'Must not save' }
			})
		).response.status,
		404
	);
