...
function App() {
	...
	const run = async () => {
		...
		try {
			...
			const loadsBeforeSave = sourceLoads;
			button('Save address').click();
			button('Save address').click();
			await wait(600);
			...
			await waitUntil(() => sourceLoads > loadsBeforeSave && button('Save address').getAttribute('aria-busy') !== 'true', 'Saved form did not refresh source');
