...
const component = {
	...
	crystal: {
		args: [
			...
			{ name: 'panelType', type: 'string', default: 'modal' },
		...
		render: {
			...
			children: [
				...
				{ tag: 'tt-countdown', props: { value: 3 } },
				{ tag: 'tt-dialog', props: { title: 'Functional dialog', name: 'Open functional dialog', type: '{panelType}' }, children: [ui('button', { type: 'button' }, { op: 'increment', key: 'count', step: 1, max: 5 }, ['Increase inside dialog'])] },
		...
		<main style={{ maxWidth: 900, padding: 16, margin: 'auto' }}>
			...
			{['first', 'second'].map((id) => (
				<section key={id} aria-label={id} style={{ marginTop: 16 }}>
					<WebpageBlocksRenderer blocks={[{ id, type: 'component', component: 'controls', args: { panelType: id === 'second' ? 'drawer' : 'modal' } }]} componentsByRef={{ controls: component }} interactive />
				</section>
			))}
		</main>
	);
