...
import { designBundlesCsp, librarySandboxCsp, devCsp } from './scripts/csp.mjs';
...
const librarySandboxPlugin = (): Plugin => ({
  ...
  configureServer(server) {
    server.middlewares.use((req,res,next) => {
      ...
      if (path !== '/library/sandbox.html' && path !== '/library/runner.js') return next();
      const file = fileURLToPath(new URL(path.endsWith('.html') ? './public/library/sandbox.html' : './dist/library/runner.js', import.meta.url));
      ...
      res.setHeader('Content-Security-Policy',librarySandboxCsp);
      res.setHeader('Cache-Control','no-store');
