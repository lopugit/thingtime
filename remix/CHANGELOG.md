...
- **Develop-preview automation no longer breaks product-branch CI**: the
  ...
  implementation — the `deploy-develop-pr-preview.mjs` controller lives on that
  control plane — and the thin workflow-caller contract rejects every local
  Actions script on product branches, so the `test:web-ci-context` and
  `test:workflow-callers` contract commands are active again in the required
  unit-test aggregate. — Codex (AI), 2026-08-10
