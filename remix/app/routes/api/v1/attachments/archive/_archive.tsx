...
import { ARCHIVE_ID_PATTERN } from '~/api/utils/attachments/attachmentArchiveCore';
...
export const createAttachmentArchiveLoader = (overrides: Partial<ArchiveDependencies> = {}) => {
	...
	return async ({ request }: { request: Request }) =>
		withAttachmentPrivateResponse(async () => {
			...
			const limit = await dependencies.enforceLimit(request, 'attachments.archive', user ? `user:${user.id}` : null, { failClosed: true });
			if (!limit.allowed) {
			...
			const plan = await dependencies.plan(viewer, id, { sharedRoot, ...(user?.isAdmin ? { isAdmin: true } : {}) });
			if (plan.ok === false) return json({ ok: false, error: plan.error }, { status: plan.status });
			if (url.searchParams.get('manifest') === '1') return json(dependencies.manifest(plan));
			const headers = {
			...
			// without spending a full archive build.
			if (request.method === 'HEAD') return new Response(null, { status: 200, headers });
			return new Response(dependencies.stream(plan, request.signal), { status: 200, headers });
		});
};
