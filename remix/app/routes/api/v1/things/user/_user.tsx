...
// to what the viewer may see (owners see all their circles, others see their feed-authorized audiences).
export const loader = async ({ request }: { request: Request }) => {
  ...
  const result = await listUserPosts(
    withFoundPostBrowser(viewerOf(user, auth.actor.pat), request),
    params.get('username') || '',
    params.get('cursor'),
  ...
  return json({ ok: true, posts: result.posts, nextCursor: result.nextCursor, postCount: result.postCount }, { headers: { 'Cache-Control': 'private, no-store', Vary: 'Cookie, Authorization' } });
};
