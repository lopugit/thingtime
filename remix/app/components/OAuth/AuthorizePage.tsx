...
const fetchJson = async (url: string, init: RequestInit = {}) => {
  ...
  return response.json().catch(() => ({ ok: false, error: 'Unexpected response' }));
};
...
export const AuthorizePage = () => {
  ...
  React.useEffect(() => {
    ...
    fetchJson(`/api/v1/apps/public?clientId=${encodeURIComponent(clientId)}&origin=${encodeURIComponent(origin)}`).then(
      (resp) => {
        if (resp?.ok && resp.app) {
          setApp(resp.app);
  ...
  const approve = async () => {
    ...
    const resp = await fetchJson('/api/v1/oauth/authorize', {
      ...
      body: JSON.stringify({ clientId: app.clientId, origin: verifiedOrigin })
    });
  ...
  if (invalidReason) {
    ...
    body = (
      <Flex sx={cardSx}>
        ...
        <Flex flexDirection="column" gap={2} fontSize="14px" paddingY={1}>
          <Flex gap={2} alignItems="center">
            {user.avatarUrl ? (
              <Image src={user.avatarUrl} alt="" boxSize="18px" borderRadius="full" />
            ) : (
              <Box>👤</Box>
            )}
            <Box>It will see your public profile — username, display name, avatar.</Box>
          </Flex>
          <Flex gap={2} alignItems="center">
            <Box>📦</Box>
            <Box>It can store its own data for you in your Thingtime account — nothing else.</Box>
          </Flex>
          <Flex gap={2} alignItems="center">
            <Box>🔒</Box>
            <Box>It can’t read your posts, email, or anything another app stored.</Box>
          </Flex>
        </Flex>
