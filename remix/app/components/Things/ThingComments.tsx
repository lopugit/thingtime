...
export function ThingComments({ thingId, linkKey = '', initialPost, description, onCommentAdded }: { thingId: string; linkKey?: string; initialPost?: PublicPost | null; description?: string; onCommentAdded?: () => void | Promise<unknown> }) {
  const user = useCurrentUser();
  return <Discussion key={`${user?.id || 'anonymous'}:${thingId}:${linkKey}`} thingId={thingId} linkKey={linkKey} initialPost={initialPost} description={description} onCommentAdded={onCommentAdded} />;
}
function Discussion({ thingId, linkKey, initialPost, description, onCommentAdded }: { thingId: string; linkKey: string; initialPost?: PublicPost | null; description?: string; onCommentAdded?: () => void | Promise<unknown> }) {
  const api = useApi();
  ...
  return <Box minW={0} data-testid="thing-comments" data-discussion-source={thingId}>
    ...
    {post && <SharedMediaProvider linkKey={linkKey}><PostCard post={post} onChanged={onChanged} onCommentAdded={onCommentAdded} defaultCommentsOpen discussionOnly /></SharedMediaProvider>}
