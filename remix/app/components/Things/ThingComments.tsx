...
export function ThingComments({
	thingId,
	linkKey = '',
	description,
	onCommentAdded,
	collectionControls = false
}: {
	thingId: string;
	linkKey?: string;
	description?: string;
	...
	return (
		<Discussion
			key={`${user?.id || 'anonymous'}:${thingId}:${linkKey}`}
			thingId={thingId}
			linkKey={linkKey}
...
function Discussion({
		...
		async (after?: string) => {
			...
			try {
				...
				setComments((previous) => [
					...new Map<string, CommentThing>(
						[...fresh.current.values(), ...(after ? previous : []), ...response.things].map((item) => [item.id, item])
					).values()
				]);
				if (after && response.nextCursor === after) throw new Error('Comment pagination did not advance. Refresh comments to retry.');
				setCursor(response.nextCursor || null);
				setLoaded(true);
			} catch (failure) {
				if (live.current && epoch === generation.current) {
					if ([401, 403, 404].includes(Number((failure as any)?.status))) {
						setComments([]);
						fresh.current.clear();
						setLoaded(false);
					}
				...
				if (live.current && epoch === generation.current) setLoading(false);
			}
		},
		[thingId, linkKey]
	);
	...
	const submit = async () => {
		...
		try {
			...
			const item: CommentThing = {
				id: response.comment.id,
				author: response.comment.author,
				crystal: { text: response.comment.text },
			...
			if (live.current) setError(failure instanceof Error ? failure.message : 'Could not post comment.');
		} finally {
			posting.current = false;
			if (live.current) setSending(false);
		}
	};
	const renderComments = (items: CommentThing[]) =>
		items.map((comment) => (
			<Box key={comment.id} borderTop="1px solid var(--tt-border)" pt={3} minW={0}>
				<Flex gap={2} wrap="wrap" fontSize="xs" color="var(--tt-muted)">
					<Text>{comment.author?.username ? `@${comment.author.username}` : 'Account unavailable'}</Text>
					<Text as="time" dateTime={comment.createdAt}>
						{new Date(comment.createdAt).toLocaleString()}
					</Text>
					<Link to={`/thing/${encodeURIComponent(comment.id)}${linkKey ? `?key=${encodeURIComponent(linkKey)}` : ''}`}>Open comment / replies</Link>
				</Flex>
				<Box mt={2} overflowWrap="anywhere">
					{comment.crystal.richText?.blocks ? (
						<RichTextBlocks blocks={comment.crystal.richText.blocks} bodyFontSize="sm" />
					) : (
						<Text whiteSpace="pre-wrap">{String(comment.crystal.text || '')}</Text>
					)}
				</Box>
			</Box>
		));
	return (
		<Box {...CARD_STYLES} p={{ base: 4, md: 6 }} minW={0} data-testid="thing-comments">
			<Flex justify="space-between" align="center" gap={2} wrap="wrap">
				<Heading as="h2" size="sm">
					Comments
				</Heading>
				<Button size="sm" variant="ghost" isDisabled={loading || sending} onClick={() => void load()}>
					Refresh comments
				</Button>
			</Flex>
			<Text color="var(--tt-muted)" fontSize="sm" mt={2}>
				{description || 'A separate discussion linked to this Thing. Comments follow its visibility and do not change its data.'}
			</Text>
			<Stack spacing={4} mt={4}>
				{collectionControls ? (
					<CollectionList
						...
						items={comments}
						searchText={(comment) =>
							`${comment.author?.username || ''} ${comment.crystal.text || ''} ${
								comment.crystal.richText ? JSON.stringify(comment.crystal.richText) : ''
							}`
						}
						hasMore={!!cursor}
				...
				{!collectionControls && loaded && !comments.length ? (
					<Text color="var(--tt-muted)" fontSize="sm">
						No comments yet.
					</Text>
				) : null}
				{!collectionControls && cursor ? (
					<Button variant="outline" isDisabled={loading} onClick={() => void load(cursor)}>
						Load older comments
					</Button>
				) : null}
				{error ? (
				...
				{user ? (
					...
					>
						...
						/>
						<Button mt={2} type="submit" isDisabled={sending || !draft.trim()}>
							{sending ? 'Posting…' : 'Post comment'}
						</Button>
					</Box>
				) : (
					<Text fontSize="sm">Sign in to add a comment.</Text>
				)}
			</Stack>
		</Box>
	);
