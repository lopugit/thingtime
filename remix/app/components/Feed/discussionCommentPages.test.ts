...
test('older cursor pages remain searchable and reachable beyond the first 20 without duplicate rows', async () => {
  ...
  const fetch = async (cursor?: string) => { calls.push(cursor); return cursor ? response([first[19], ...old], null) : response(first, 'cp1.Qk9VTkRBUlk._N6--ciphertext'); };
  ...
  assert.deepEqual(calls, [undefined, 'cp1.Qk9VTkRBUlk._N6--ciphertext']);
...
test('empty authorized pages advance to later matching comments', async () => {
  const pager = createDiscussionCommentPager();
  const pages = [response([], 'cp1.Vmlld2Vy._Qm91bmRhcnk-A'), response([], 'cp1.Vmlld2Vy._Qm91bmRhcnk-B'), response([comment('needle')], null)];
  ...
  assert.deepEqual(seen, [undefined, 'cp1.Vmlld2Vy._Qm91bmRhcnk-A', 'cp1.Vmlld2Vy._Qm91bmRhcnk-B']);
  assert.equal(rows[0]?.id, 'needle');
});

for (const cycle of [['cp1.Vmlld2Vy._Qm91bmRhcnk-A', 'cp1.Vmlld2Vy._Qm91bmRhcnk-A'], ['cp1.Vmlld2Vy._Qm91bmRhcnk-C', 'cp1.Vmlld2Vy._Qm91bmRhcnk-D', 'cp1.Vmlld2Vy._Qm91bmRhcnk-C']]) {
...
test('transport failures retry the same cursor without skipping an older page', async () => {
  const pager = createDiscussionCommentPager();
  await pager.load(async () => response([], 'cp1.Vmlld2Vy._Qm91bmRhcnk-E'));
  await assert.rejects(pager.load(async () => { throw new Error('Offline'); }), /Offline/);
  assert.equal(pager.hasMore, true);
  const page = await pager.load(async cursor => { assert.equal(cursor, 'cp1.Vmlld2Vy._Qm91bmRhcnk-E'); return response([comment('older')], null); });
...
test('opaque tokens retain punctuation and are forwarded unchanged without interpreting their order', async () => {
  const pager = createDiscussionCommentPager();
  const tokens = ['cp1.Vmlld2Vy._Qm91bmRhcnk-Z', 'cp1.Vmlld2Vy._Qm91bmRhcnk-A'];
...
test('revocation stops the page chain and cannot be retried as an authorized empty result', async () => {
  const pager = createDiscussionCommentPager();
  await pager.load(async () => response([comment('permitted')], 'cp1.Vmlld2Vy._Qm91bmRhcnk-E'));
