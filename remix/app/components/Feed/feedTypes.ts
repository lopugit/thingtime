...
import type { PostMediaLayout, SubspaceAccessMode, SubspaceRole } from '~/schemas/registry';

export type { PostMediaLayout, SubspaceRole };

// A subspace's join/visibility mode. The registry name is SubspaceAccessMode;
// every client surface speaks of a subspace's "access", so the shorter alias is
...
// Up/down votes are a SEPARATE, deliberately limited reaction kind: exactly one
// of 'up' | 'down' per (viewer, target). They never touch the open-vocabulary
// emoji reactions, which keep their multi-token semantics. Mirrors
// PublicUpdownVotes in api/utils/things/updownCore.ts.
...
// The zero tally an unvoted target renders as — posts and comments carry
// `votes` only once the rollout reaches them, so every reader falls back here
// instead of guarding each field.
export const EMPTY_VOTES: PublicUpdownVotes = { up: 0, down: 0, score: 0, viewerVote: null };

// Subspace vocabulary carried on subspace posts — mirrors the public
// projections in api/utils/things/things.ts.

// Lean subspace embed — identity + branding + the viewer's own role there
// (never the member roster).
...
export type PublicSubspaceMod = {
  ...
  // mod page's Reports queue) — absent for everyone else
  reportCount?: number;
};
