...
test('notification controls require compatible origin-scoped semantic capabilities', () => {
  ...
  assert.equal(supportsNotificationTools({ ...manifest, features: { ...manifest.features, 'api.notifications-test': '1.1.1' } }, origin), false);
...
test('new operations are deliberately registered in both manifests', () => {
  for (const [id, version] of [['notifications-test', '1.2.0'], ['lopu-reminders', '1.1.0'], ['watch-recordings', '1.0.0']]) {
