...
import { SETTINGS_TABS, settingsTabHref, type SettingsTab } from './settingsTabs';
import { DesktopSettings } from './DesktopSettings';
import { DrawerItemSettings } from './DrawerItemSettings';
...
// Shared settings content for the dedicated page and drawer popup.
		...
		<Flex flexDirection="column" rowGap={4}>
			...
			<Flex flexDirection="column" rowGap={1}>
				...
				<Text fontSize="11px" color="var(--tt-muted, #9a9aa6)">
					Private — never shown on your profile. Apps you log in to only see it if you approve the birthday permission on their consent screen.
				</Text>
			</Flex>
...
export const SettingsContent = ({
	...
	React.useEffect(() => {
		...
		const aim = () => {
			if (cancelled) return;
			const target = contentRef.current?.querySelector<HTMLElement>(`[id="${CSS.escape(id)}"]`);
			if (target) {
	...
	return (
		...
		>
			<Flex as="nav" aria-label="Settings categories" gap={2} wrap="wrap">
				{SETTINGS_TABS.filter((item) => item.id !== 'admin' || user?.isAdmin).map((item) => (
					<Button
						...
						key={item.id}
						size="sm"
						variant={tab === item.id ? 'solid' : 'outline'}
							...
							<SettingsSection eyebrow="Privacy" description="Choose what other people can see on your public profile.">
								<Flex flexDirection="column">
									<SettingRow
										...
										hint="On by default. Your email always stays private from other people; this controls whether you see it on your own profile page."
									>
										<Switch
							...
							<SettingsSection eyebrow="Security" description="Extra checks between a password and a session.">
								<Flex flexDirection="column">
									<SettingRow
										label="Email 2FA 🔐"
										hint={
											user.emailVerified ? 'Logins also need a 6-digit code emailed to you' : 'Verify your email first — codes are delivered there'
										}
									>
										<Switch
						...
						{user && !user.temporary && (
							<Box id="secure-vault" width="100%" scrollMarginTop="calc(var(--tt-nav-clearance, 54px) + 16px)">
								<SettingsSection
									...
									description="Encrypted passwords, keys, environments, and the AI provider connections Lopu can use for this account."
								>
            ...
            #lopu-credits. */}
				...
				{tab === 'admin' && !user?.isAdmin && <Text>Admin settings are available to administrators.</Text>}
			</Box>
		</Flex>
	);
};
