...
export async function runLibraryRequest(example: LibraryExample, input: Record<string, unknown>, apiKey: string, transport: typeof fetch = fetch) {
	apiKey = apiKey.trim();
	const { url, headers } = buildExampleRequest(example, input, apiKey);
	const response = await transport(url, { method: 'GET', headers, redirect: 'error', credentials: 'omit', signal: AbortSignal.timeout(12000) });
