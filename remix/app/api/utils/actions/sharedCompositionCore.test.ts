...
test('saved action references use defaults, saved args and every persisted page instance', () => {
	const crystal = {
		args: [{ name: 'action', type: 'string', default: 'default-action' }],
		savedArgs: { action: 'saved-action' },
		render: { ttAction: '{action}' }
	};
	assert.deepEqual(
		compositionReferences(['component'], crystal).map(({ ref }) => ref),
		['saved-action']
	);
	assert.deepEqual(
		compositionReferences(['component'], crystal, { action: 'page-action' }).map(({ ref }) => ref),
		['page-action']
	);
	assert.deepEqual(
		compositionReferences(['component'], { ...crystal, savedArgs: {} }).map(({ ref }) => ref),
		['default-action']
	);
	assert.equal(
		compositionReferences(['webpage'], {
			blocks: [
...
test('saved action discovery covers inactive branches and repeat scopes without executing argument data', () => {
	const crystal = {
		savedArgs: { prefix: 'draw', query: 'private', index: 'private', payload: { ttAction: 'private' } },
		render: {
			children: [
...
test('composition edges follow nested stored controls but never input or metadata references', () => {
	const refs = compositionReferences(['component'], {
		...
		render: {
			children: [
				{ ttIf: { then: { ttAction: 'draw', ttActionInputs: { ttAction: 'private-secret' } }, else: { ttEach: { node: { ttAction: 'again' } } } } }
			]
		},
...
test('dynamic user-controlled references do not grant inherited access', () => {
	...
	assert.deepEqual(
		compositionReferences(['action'], {
			steps: [
				{ op: 'things.get', id: '$input.id' },
				{ op: 'things.get', id: 'stored-id' },
				{ op: 'actions.invoke', action: 'child' }
			]
		}),
		[
			{ kind: 'data', ref: 'stored-id' },
			{ kind: 'action', ref: 'child' }
		]
	);
});
...
test('Chakra rawChildren controls are part of the same composition graph', () => {
	assert.deepEqual(compositionReferences(['component'], { render: { chakra: 'Box', rawChildren: [{ chakra: 'Button', ttAction: 'draw' }] } }), [
		{ kind: 'action', ref: 'draw' }
	]);
});

test('schema templates include stored actions but not field values or action inputs', () => {
	assert.deepEqual(
		compositionReferences(['schema'], {
...
test('collection discovery follows authored row controls while refusing row-data authority', () => {
	const crystal = {
		savedArgs: { item: { action: 'private' } },
		render: {
			tag: 'tt-collection',
			props: { itemsPath: 'result.rows', itemTemplate: { ttTemplate: { children: [{ ttAction: 'save' }, { ttAction: '{item.action}' }] } } }
		}
	};
	assert.deepEqual(compositionReferences(['component'], crystal), [{ kind: 'action', ref: 'save' }]);
	assert.deepEqual(compositionReferences(['component'], { render: { tag: 'div', props: crystal.render.props } }), []);
	assert.deepEqual(compositionReferences(['component'], { render: { tag: 'tt-dialog', props: { closeOnAction: 'private' } } }), []);
});
