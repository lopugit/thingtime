...
test('deferred collection rows copy stored media while runtime rows and arbitrary metadata grant no access', () => {
	for (const tag of [{ tag: 'tt-collection' }, { chakra: 'Collection' }]) {
		const source = {
			savedArgs: { prefix: 'att_', image: 'source', item: { secret: 'private' } },
			render: {
				...tag,
				props: {
					...
					itemTemplate: {
						ttTemplate: {
							tag: 'div',
							children: [
								{ tag: 'img', props: { src: '/api/v1/attachments/content?id={prefix}{image}' } },
								{ tag: 'img', props: { src: '/api/v1/attachments/content?id={item.secret}' } }
							]
						}
					},
					metadata: { tag: 'img', props: { src: '/api/v1/attachments/content?id=unrelated' } }
				}
			}
		};
		assert.deepEqual([...compositionAttachmentIds(['component'], source)], ['att_source']);
		const result = bindCopiedTemplateMedia(source, copies, ['att_source']);
		const outer: any = resolveTemplate(result.render, result.savedArgs);
		const row: any = resolveTemplate(outer.props.itemTemplate, { ...result.savedArgs, item: { secret: 'row-owned' } });
		assert.equal(row.children[0].props.src, copied);
		assert.equal(row.children[1].props.src, '/api/v1/attachments/content?id=row-owned');
		...
		const rerender: any = resolveTemplate(again.render, again.savedArgs);
		assert.equal(
			(resolveTemplate(rerender.props.itemTemplate, again.savedArgs) as any).children[0].props.src,
			'/api/v1/attachments/content?id=att_again'
		);
			...
			children: [
				{
					ttEach: {
						...
						node: {
							tag: 'img',
							props: {
								src: '/api/v1/attachments/content?id={prefix}{item}',
								title: '{prefix}{item}',
...
test('late bindings only map rendered first-party URLs, one hop, not unused references or external/keyed URLs', () => {
	const source = {
		render: {
			...
			children: [
				{ tag: 'img', props: { src: old } },
				{ tag: 'img', props: { src: `https://outside.test${old}` } },
				{ tag: 'img', props: { src: `${old}&key=secret` } },
				{ tag: 'img', props: { src: '/api/v1/attachments/content?id={runtime}' } }
			]
		}
	};
...
test('fork maps exact attachment IDs in persisted arguments, defaults, nested lists and explicit instance contexts', () => {
	...
	const source = {
		title: 'att_source',
		args: [{ name: 'image', label: 'att_source', default: 'att_source' }],
		savedArgs: { image: 'att_source', images: [{ id: 'att_source', caption: 'Look at att_source' }] },
		render,
...
test('copied attachment-ID arguments still select their authored map and equality branches', () => {
			...
			children: [
				{
					ttMap: {
						arg: 'image',
						values: { att_source: { tag: 'img', props: { src: '/api/v1/attachments/content?id={image}' } } },
						default: { tag: 'p', children: ['wrong map branch'] }
					}
				},
				{
					ttIf: {
						arg: 'image',
						equals: 'att_source',
						then: { tag: 'p', children: ['selected image'] },
						else: { tag: 'p', children: ['wrong equality branch'] }
					}
				},
	...
	const output = rewriteCopiedAttachmentReferences(source, copies);
	assert.deepEqual(resolveTemplate(output.render, output.savedArgs), {
		tag: 'div',
		children: [
			{ tag: 'img', props: { src: copied } },
			{ tag: 'p', children: ['selected image'] },
			{ tag: 'p', children: ['in image list'] }
		]
	});
...
test('fork media rewrites exact URL scalars, saved arguments, CSS and parsed HTML without mutating the source', () => {
	const source = {
		savedArgs: { image: old },
		args: [{ name: 'image', default: old }],
		blocks: [
			{ type: 'media', src: old },
			{
				type: 'container',
				css: { backgroundImage: `url("${old}")`, _hover: { maskImage: `url(${old})` } },
				children: [{ type: 'html', html: `<img src="${old}"><p style='background: url("${old}")'>Hello</p>` }]
			}
		]
	};
	const output = rewriteCopiedAttachmentReferences(source, copies);
...
test('fork media preserves unrelated URLs, keyed URLs, unresolved templates and prose', () => {
	const source = {
		...
		keyed: `${old}&key=secret`,
		shared: `${old}&sharedRoot=root`,
		unresolved: '/api/v1/attachments/content?id={image}',
		other: '/api/v1/attachments/content?id=att_other',
		prose: `The picture is ${old}`,
		label: `A url(${old}) example`
	};
...
test('HTML retargeting uses browser attribute parsing and preserves non-rendering source bytes', () => {
	const html = `<!doctype html><html><head><title>${old}</title></head><body><!-- ${old} --><script>"${old}"</script><p data-url="${old}">${old}</p><IMG SRC='${old.replace(
		'?',
		'?unused=1&amp;'
	)}'></body></html>`;
...
test('HTML CSS rewriting preserves rejected declarations and only maps safe declaration URLs', () => {
	...
	assert.equal(
		mapAuthoredHtmlMedia(source, (value) => value),
		source
	);
});
