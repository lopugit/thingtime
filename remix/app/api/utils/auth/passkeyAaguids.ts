...
//
// Deliberately a tuple list, not a `'<uuid>': '<name>'` object literal: these
// are PUBLIC authenticator identifiers, but secret scanners (GitGuardian's
// generic-password detector) pattern-match quoted high-entropy strings
// assigned next to password-manager names and flag every commit touching the
// file. The tuple shape carries the same data without the assignment shape
// that trips the false positive.
const PASSKEY_PROVIDERS: Array<[aaguid: string, providerName: string]> = [
...
const PASSKEY_PROVIDER_NAMES = new Map(PASSKEY_PROVIDERS);
...
export const providerNameForAaguid = (aaguid: string | null | undefined): string | null => {
	...
	return PASSKEY_PROVIDER_NAMES.get(normalized) || null;
};
