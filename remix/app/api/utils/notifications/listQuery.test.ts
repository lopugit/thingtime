...
test('category expands to its types and intersects with an explicit type list', () => {
  assert.deepEqual(resolveNotificationListQuery({ category: 'system' }).types, ['action-run', 'recording-reminder', 'lopu-reminder', 'lopu-message', 'login-success', 'system-message']);
...
test('disabled types are excluded, explicit types are intersected with the enabled set', () => {
  ...
  assert.deepEqual(buildNotificationListFilters('u1', prefs, resolveNotificationListQuery({ category: 'system' }))?.base['crystal.type'], { $in: ['recording-reminder', 'lopu-reminder', 'lopu-message', 'login-success', 'system-message'] });
  const allSystemOff = normalizeNotificationPrefs({ 'action-run': false, 'recording-reminder': false, 'lopu-reminder': false, 'lopu-message': false, 'login-success': false, 'system-message': false });
...
test('history includes muted types and ignores channel masters without widening ownership', () => {
  ...
  assert.deepEqual(buildNotificationListFilters('owner', prefs, query)?.base, {
    ...
    'crystal.type': { $in: ['action-run', 'recording-reminder', 'lopu-reminder', 'lopu-message', 'login-success', 'system-message'] }
  });
