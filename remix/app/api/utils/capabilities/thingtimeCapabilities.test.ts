...
test('Thingtime capability manifest is origin scoped and covers the generated API route map', () => {
  ...
  assert.equal(manifest.schemaVersion, 1);
	...
	assert.equal(manifest.features['api.actions-run']?.version, '1.3.0');
	assert.equal(manifest.features['api.things-fork']?.version, '1.2.0');
  ...
  assert.equal(manifest.features['api.things']?.version, '1.9.0');
  ...
  assert.equal(manifest.features['api.lopu-recordings-personal']?.version, '1.0.0');
  ...
  assert.equal(manifest.features['api.things-update']?.version, '1.2.5');
