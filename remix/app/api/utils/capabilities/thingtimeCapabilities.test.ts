...
test('Thingtime capability manifest is origin scoped and covers the generated API route map', () => {
  ...
  assert.equal(manifest.schemaVersion, 1);
	...
	assert.equal(manifest.features['api.things-export']?.version, '1.7.1');
  for (const [feature, required, previous] of [['api.things-export', '1.7.1', '1.7.0'], ['api.attachment-content', '1.6.4', '1.6.3']]) {
