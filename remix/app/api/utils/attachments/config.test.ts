...
test('private S3 config accepts only private role, bucket, and region values', () => {
	assert.deepEqual(
		getPrivateS3Config({
			THINGTIME_PRIVATE_S3_ROLE_ARN: 'arn:aws:iam::232261468846:role/thingtime-vercel-s3',
			THINGTIME_PRIVATE_S3_BUCKET: 'thingtime-private-example',
			THINGTIME_PRIVATE_S3_REGION: 'ap-southeast-2'
		} as NodeJS.ProcessEnv),
		{
			roleArn: 'arn:aws:iam::232261468846:role/thingtime-vercel-s3',
...
test('private S3 config rejects missing, dotted, and generic credential-shaped setup', () => {
	for (const env of [
		{},
		{
			THINGTIME_PRIVATE_S3_ROLE_ARN: 'arn:aws:iam::232261468846:role/thingtime',
