...
export const exchangeDesktopAuthorizationCode = async (input: {
  ...
  if (meta.clientId !== clientId || meta.redirectUri !== redirect.uri || meta.origin !== redirect.origin) {
    return invalidGrant();
  }
  if (meta.codeChallengeMethod !== 'S256' || !pkceVerifierMatches(verifier, meta.codeChallenge)) {
    return invalidGrant();
  }
  ...
  const sharedThings = Array.isArray(meta.sharedThings)
    ? meta.sharedThings.filter((id: unknown): id is string => typeof id === 'string')
    : [];
