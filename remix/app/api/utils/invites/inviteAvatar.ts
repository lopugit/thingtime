...
export const normalizeInviteAvatar = async (value: unknown): Promise<string | null> => {
	...
	if (typeof value !== 'string' || value.length > 112_000 || !/^data:image\/(jpeg|png|webp);base64,[A-Za-z0-9+/]+=*$/.test(value))
	...
	try {
		...
		bytes = await image.rotate().resize(128, 128, { fit: 'cover' }).png({ compressionLevel: 9, palette: false }).toBuffer();
	...
	if (choice.kind === 'provider') {
		let verdict;
		try {
			verdict = await choice.provider.analyzeImage({ bytes, contentType: 'image/png', filename: 'invite-avatar.png' });
		} catch (error) {
