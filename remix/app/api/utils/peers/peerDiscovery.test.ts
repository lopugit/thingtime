...
test('peer request signatures bind the raw body, method and route with a bounded timestamp', () => {
	...
	const headers = {
		...
		'x-thingtime-peer-signature': signPeerRequest(process.env.THINGTIME_PEER_DISCOVERY_SECRET, 'POST', '/api/v1/peers', timestamp, body),
...
test('peer stream events are independently signed by the pinned deployment key', () => {
	...
	assert.ok(identity);
	const event = signedPeerStreamEvent(identity, 'https://thingtime.com', { type: 'peer', peer: { origin: 'https://thingtime.com' } });
	assert.deepEqual(verifyPeerStreamEvent(event, 'https://thingtime.com')?.event, { type: 'peer', peer: { origin: 'https://thingtime.com' } });
	assert.equal(verifyPeerStreamEvent({ ...event, peer: { origin: 'https://evil.example' } }, 'https://thingtime.com'), null);
});
