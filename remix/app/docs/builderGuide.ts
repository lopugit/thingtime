...
export const builderGuideSections = [
	...
	},
	{
		...
		paragraphs: [
			...
			'For a local draft, bind value to { ttArg: "draftName", fallback: "result.record.name" }. The fallback is another safe scope path used only when the draft is undefined; empty text, false, zero and null stay deliberate values. A $ui set binding updates the draft. Its optional clear list empties related draft fields, such as a place ID when the address is typed. A $ui patch with values updates several scalar fields atomically. Both obey the same 32-key instance limit and cannot replace viewer, result or other runtime authority. Source refreshes retain drafts; explicit URL-query navigation starts a new instance.',
	...
	},
	{
		...
		paragraphs: [
			...
			'For incremental records, give the collection source: { action, inputs?, itemsPath?, cursorPath?, cursorInput?, itemKey? }. The Action is a literal saved reference and returns { items, nextCursor } by default. Each row needs a stable id; custom paths can select nested fields. Next, search and infinite scrolling fetch more pages, including empty intermediate pages, and deduplicate row keys. Requests run as the viewer or through the existing read-only shared boundary. The Action retains endpoint permissions and domain logic. Copies/export include this Action. Refresh keeps visible rows until its replacement arrives; target/account changes isolate prior results. Failures preserve rows and offer retry, while denied access clears them. Repeated cursors, more than 200 pages, 10,000 rows or 4 MB stop visibly. Ordinary array collections still use itemsPath without source. filterPath/filterValue constrain displayed rows without discarding the cursor; countStateKey exposes the loaded source count through bounded local UI state for editable summaries and empty hints.',
			'tt-menu renders editable links, Action buttons and tt-dialog triggers in a native top-layer popup. name supplies the accessible trigger label; triggerContent uses the same sanitized HTML/SVG renderer; className styles the trigger and contentClassName styles the popup. disabled prevents opening. Outer tt-style rules continue to apply. The primitive positions the popup within the viewport and supplies keyboard navigation, typeahead, light dismissal and focus return. Dialogs launched inside a menu render beside its hidden popup, keeping the surrounding Component style, Action and form scope.',
			...
			'tt-dialog (Chakra Dialog) can autoOpen on a route, return through closeQuery on Escape or Close, and close after a successful closeOnAction. Copying rewrites that Action reference along with the executable form. Failed saves keep the dialog open. className lets an enclosing style namespace supply the dialog appearance. closeDisabled blocks manual dismissal while saving; a successful closeOnAction still closes the open dialog. Optional triggerContent and closeContent supply sanitized HTML/SVG markup for the trigger and close button; role: alertdialog identifies a confirmation; closeLabel supplies its accessible name.',
			'Live Component templates expose page.id for the current live or Builder page (null outside a page), so a reusable control does not hard-code its containing page. The API still authorizes every requested page operation. They also expose read-only pending and pendingAction while a saved Action is awaiting confirmation or running. Bind labels and disabled states to them; tt-form accepts disabled to pause its fields without resetting its identity, revision or draft. These values belong to the current Component instance and cannot be overwritten by $ui.',
			'Use tt-form (Chakra Form) as a fieldset with identityName, identity, revisionName, revision and resetKey. New forms generate one UUID; existing forms use identity. The revision is captured when the form opens. Refetches and failures preserve both, so retrying an ambiguous create can use the same id and a stale edit cannot silently adopt a newer revision. By default, only an explicit resetKey change or a different identity starts another draft. The Action and API must enforce idempotency and concurrency.',
			'To reset a new tt-form after an accepted save, bind completion to last and return the form identity in result under identityName (for example result.shareId). Only ok: true with that exact generated identity clears the fields and starts a new operation; stale, failed, sibling and saved-record receipts preserve the draft. Optional completionState applies a bounded local scalar patch through the same rules as $ui, for example to advance a selected stage after saving. Omit completion to keep manual reset behavior. A form containing tt-attachments must also bind committedTargetId and committedIds from that same receipt: the reset discards the composer, and any uploaded attachment the receipt did not mark committed is deleted from storage even though the saved record already references it.',
			...
			'tt-attachments (Chakra Attachments) selects account-owned drafts using name, purpose (comment or post), targetId, maxFiles, imageOnly and title. It submits comma-separated IDs through the named hidden field. A saved Action commits those IDs through the appropriate API and returns them; bind committedTargetId and committedIds to that exact result so successful files are retained. Failed saves preserve drafts. Start another entry by resetting the surrounding tt-form; every reset path, including an automatic completion reset, requires those committed bindings or the saved files are removed with the discarded draft. tt-media (Chakra Media) displays the authorized attachment metadata and postId returned by a source Action.',
			'tt-attachments accepts helperText and allowLinkedUrls (true by default). Its optional readyStateKey writes a local boolean: true only when at least one attachment is ready, uploads are not blocking and the selection has not been committed. Bind a saved submit button’s disabled state to that value; the Action must still validate the submitted IDs.',
			'tt-discussion displays the standard Thingtime discussion for thingId, including rich comments, replies, reactions and media. Set description to introduce the discussion and collectionControls to false to hide search and page-size controls (shown by default). Place it inside your own Component layout and scoped styles. Inert previews never load it. All reads and writes use the current viewer and the target Thing’s live permissions; displaying a discussion does not grant access. Successful comments refresh source-bound blocks on the page.',
			'For copyable apps, use relative query links (such as ?view=detail&id={item.id}) instead of embedding the original page path. Return a stable operation tag from saved Actions when a Component needs to recognize its save result; compare last.result.operation rather than an Action key that changes during copying. Keep last.ok checks and use silent: true when a Component provides its own receipt.',
			'Relative query links retain the current page and view mode inside Builder. A $ui control with op: back and fallback params returns through the previous same-page Component link, including nested records and reloads. Direct entry, missing history or another page uses the declared query params instead. Back never uses an unrelated browser-history entry.',
			'Bind a button to $install and show it when installAvailable is true. On an ordinary shared page this opens the platform copy dialog, creates private editable copies of its referenced Things, and opens the new page. Saved browser Actions then run as the viewer; connected records retain their existing permissions. A revoked team membership therefore blocks the copied app from reading or changing those records. Seeded app suites keep their existing install behavior.',
