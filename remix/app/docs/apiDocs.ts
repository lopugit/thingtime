...
export const apiEndpointDocs: ApiEndpointDoc[] = [
	...
	endpoint({
		id: 'lopu-recordings-personal', contractVersion: '1.0.0', featureVersion: '1.0.0', group: 'lopu', title: 'Personal recording worker',
		...
		steps: ['Negotiate api.lopu-recordings-personal >=1.0.0 with matching major at the exact origin before sending a credential.', 'Pair the outbound worker and select it in /lopu/recordings.', 'Claim, download/transcribe locally, heartbeat, submit grounded text results, and retry only identical completion payloads after an uncertain response.'],
