...
export const apiEndpointDocs: ApiEndpointDoc[] = [
  ...
  endpoint({
    ...
    detail:
			'GET returns a stripped public projection that never includes email or verification fields, plus wornTheme ({id, name} of the profile owner’s active theme, resolved through the public share gate — null when unset or private). POST updates the caller display name, bio, avatar, or banner. Avatar/banner may use either one external http(s) URL or a ready private attachment created for the exact profile slot; managed media remains in the private bucket and is served through a stable same-origin content route.',
    auth: {
      mode: 'optional',
