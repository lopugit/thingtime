...
const deviceEndpointDocs: ApiEndpointDoc[] = [
	...
	endpoint({
		...
		detail:
			'Unknown kinds and unknown input fields are rejected. The typed vocabulary includes connector.start/stop, session.list/read/create/send/interrupt, approval.respond, app.focus/launch/quit/force-quit/hide/unhide/hide-others, audio routing/mute/levels, Wi-Fi connect/disconnect/power, per-display brightness/mode/origin/mirroring, default printer and preferred camera, paired Bluetooth-device and existing VPN-service connections, keep-awake, fixed Apple Music play/pause/previous/next controls, lock/sleep, restart/shutdown/logout, and screen.start/stop. Wi-Fi connection accepts only a visible SSID and uses an already-saved local credential or an open network; passwords are never accepted or persisted. Apple Music and lifecycle actions always require a new approval and resolve as needs-review after reconnect because an Apple Event can return before its external effect is observable. HDR and Low Power Mode are read-only; Focus, AirDrop, Bluetooth radio state, camera privacy, and global media playback are not exposed because macOS has no supported scoped setter. No arbitrary executable input exists. The per-account/device permission mode controls whether commands queue immediately, request one approval, or are denied. Pairing, capability, freshness, locked-session and macOS privacy checks remain required in every mode.',
		auth: { mode: 'session-or-bearer', description: 'Full Thingtime user session.' },
		methods: ['GET', 'POST'],
