...
export const apiEndpointDocs: ApiEndpointDoc[] = [
  ...
  endpoint({
    ...
    detail:
      ...
      '404 for unknown apps, 400 for malformed origins. EXCEPTION: add sandbox=1 and the lookup ' +
      'answers for ANY clientId with a mock app payload (flagged sandbox: true) so integrators can ' +
      'build the consent flow before registering — pair with POST /api/v1/oauth/sandbox for a ' +
