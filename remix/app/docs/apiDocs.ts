...
const deviceEndpointDocs: ApiEndpointDoc[] = [
	...
	endpoint({
		...
		detail:
			'Unknown kinds and unknown input fields are rejected. The typed vocabulary includes connector.start/stop, session.list/read/create/send/interrupt, approval.respond, app.focus/launch/quit/hide/unhide, system.volume.set, system.audio.mute.set, system.audio.output/input/sound-effects-output.set, system.brightness.set, system.lock, system.wifi.connect/disconnect/power.set, and screen.start/stop. Wi-Fi connection accepts only a visible SSID and uses an already-saved local credential or an open network; passwords are never accepted or persisted. Steer requires expectedTurnId; interrupt requires turnId; list/read are cursor paged with limit 1..100. No arbitrary executable input exists. The per-account/device permission mode controls whether commands queue immediately, request one approval, or are denied. Pairing, capability, freshness, locked-session and macOS privacy checks remain required in every mode.',
		auth: { mode: 'session-or-bearer', description: 'Full Thingtime user session.' },
		methods: ['GET', 'POST'],
