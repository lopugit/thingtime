...
test('emoji transfer, library and move capabilities reject pre-support origins', () => {
  ...
  for (const [feature, version] of [['api.things-export', '1.14.0'], ['api.things-import', '1.10.0'], ['api.things', '1.19.0'], ['api.things-bulk', '1.4.0']]) {
...
test('archive library and folder export reject origins without complete archive traversal', () => {
  const manifest = thingtimeCapabilityManifest('https://thingtime.test');
  for (const [feature, required, previous] of [['api.things', '1.19.0', '1.13.1'], ['api.things-export', '1.14.0', '1.9.0']]) {
  ...
  assert.equal(createApiCapabilitiesManifest().features['api.things'], '1.18.0');
  assert.equal(createApiCapabilitiesManifest().features['api.things-export'], '1.14.0');
});
...
test('shared dependency reads negotiate the additive Things contract on both manifests', () => {
	...
	assert.equal(createApiCapabilitiesManifest().features['api.things'], '1.18.0');
	assert.equal(thingtimeCapabilityManifest('https://thingtime.test').features['api.things'].version, '1.19.0');
...
test('persistent attachment content and resized previews advertise their additive contract', () => {
	assert.equal(createApiCapabilitiesManifest().features['api.attachment-content'], '1.7.1');
	assert.equal(thingtimeCapabilityManifest('https://thingtime.test').features['api.attachment-content'].version, '1.7.1');
...
test('shared audience-boundary corrections reject pre-fix and incompatible origins', () => {
	...
	for (const [name, required, previous] of [
		['things', '1.17.1', '1.15.0'], ['things-update', '1.2.6', '1.2.5'],
		['actions-run', '1.3.1', '1.3.0'], ['things-fork', '1.5.0', '1.4.0'], ['attachment-content', '1.7.1', '1.6.4']
	]) {
...
test('subspace user flairs publish their contract versions', () => {
	...
	for (const feature of ['api.things-comment', 'api.things-user']) {
		assert.equal(manifest.features[feature], feature === 'api.things-comment' ? '1.5.1' : '1.5.0', feature);
	}
	// (S7 moved the single read on to 1.4.0 — commentSort=top|new|old; the
	// shared projection's other three ids are untouched)
	assert.equal(manifest.features['api.things'], '1.18.0');
...
test('Lopu continuation clients reject origins without checkpoint and renewable-task contracts', () => {
 ...
 const manifest = thingtimeCapabilityManifest('https://thingtime.test');
 for (const [feature, required, previous] of [['api.actions-run', '1.5.0', '1.4.0'], ['api.things', '1.19.0', '1.18.0'], ['api.things-update', '1.4.0', '1.3.0']]) {
