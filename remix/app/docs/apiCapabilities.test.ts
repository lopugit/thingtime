...
test('archive re-export is advertised by both manifests and rejects incomplete older implementations', () => {
  assert.equal(createApiCapabilitiesManifest().features['api.things-export'], '1.9.0');
  assert.equal(thingtimeCapabilityManifest('https://thingtime.test').features['api.things-export'].version, '1.9.0');
...
test('shared dependency reads negotiate the additive Things contract on both manifests', () => {
	...
	assert.equal(thingtimeCapabilityManifest('https://thingtime.test').features['api.things'].version, '1.13.1');
...
test('shared audience-boundary corrections reject pre-fix and incompatible origins', () => {
	...
	for (const [name, required, previous] of [
		['things', '1.13.1', '1.12.1'], ['things-update', '1.2.6', '1.2.5'],
...
test('subspace user flairs publish their contract versions', () => {
	...
	assert.equal(manifest.features['api.things'], '1.12.1');
