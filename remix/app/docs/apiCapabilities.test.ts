...
test('shared dependency reads negotiate the additive Things contract on both manifests', () => {
	assert.equal(createApiCapabilitiesManifest().features['api.things'], '1.6.5');
	assert.equal(thingtimeCapabilityManifest('https://thingtime.test').features['api.things'].version, '1.7.5');
...
test('subspace user flairs publish their contract versions', () => {
	...
	assert.equal(manifest.features['api.things'], '1.6.5');
