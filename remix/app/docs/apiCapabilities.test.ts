...
test('emoji transfer, library and move capabilities reject pre-support origins', () => {
  ...
  for (const [feature, version] of [['api.things-export', '1.14.0'], ['api.things-import', '1.10.0'], ['api.things', '1.26.0'], ['api.things-bulk', '1.4.0']]) {
...
test('archive library and folder export reject origins without complete archive traversal', () => {
  const manifest = thingtimeCapabilityManifest('https://thingtime.test');
  for (const [feature, required, previous] of [['api.things', '1.26.0', '1.13.1'], ['api.things-export', '1.14.0', '1.9.0']]) {
  ...
  assert.equal(createApiCapabilitiesManifest().features['api.things'], '1.26.0');
  assert.equal(createApiCapabilitiesManifest().features['api.things-export'], '1.14.0');
});
...
test('shared dependency reads negotiate the additive Things contract on both manifests', () => {
	...
	assert.equal(createApiCapabilitiesManifest().features['api.things'], '1.26.0');
	assert.equal(thingtimeCapabilityManifest('https://thingtime.test').features['api.things'].version, '1.26.0');
...
test('capabilities publish the native Apple notification device contract', () => {
	...
	assert.equal(manifest.features['api.attachment-uploads'], '1.4.1');
...
test('persistent attachment content and resized previews advertise their additive contract', () => {
	assert.equal(createApiCapabilitiesManifest().features['api.attachment-content'], '1.10.0');
	assert.equal(thingtimeCapabilityManifest('https://thingtime.test').features['api.attachment-content'].version, '1.10.0');
...
test('subspace user flairs publish their contract versions', () => {
	...
	assert.equal(manifest.features['api.things'], '1.26.0');
...
test('Lopu continuation clients reject origins without checkpoint and renewable-task contracts', () => {
 ...
 const manifest = thingtimeCapabilityManifest('https://thingtime.test');
 for (const [feature, required, previous] of [['api.actions-run', '1.6.0', '1.4.0'], ['api.things', '1.26.0', '1.19.0'], ['api.things-update', '1.4.0', '1.3.0']]) {
...
test('uncapped post attachments and profile grants negotiate their own feature contracts', () => {
  ...
  for (const [feature, version, previous] of [['api.things', '1.26.0', '1.18.0'], ['api.things-user', '1.6.0', '1.5.0']]) {
...
test('comment media readers require the attachment metadata projection', () => {
 ...
 assert.equal(capabilitySatisfies(version, '1.24.0'), true);
 for (const actual of [undefined, '1.23.0', '2.0.0']) assert.equal(capabilitySatisfies(actual, '1.24.0'), false);
});

test('cursor discussion clients require the additive1.26 contract while media keeps1.24 compatibility', () => {
  const source = createApiCapabilitiesManifest();
  const origin = thingtimeCapabilityManifest('https://discussion.test');
  assert.equal(source.features['api.things'], '1.26.0');
  assert.equal(origin.features['api.things'].version, '1.26.0');
  for (const old of [undefined, '1.24.0', '1.25.0', '2.0.0']) assert.equal(capabilitySatisfies(old, '1.26.0'), false);
  for (const current of ['1.26.0', '1.26.1', '1.27.0']) assert.equal(capabilitySatisfies(current, '1.26.0'), true);
