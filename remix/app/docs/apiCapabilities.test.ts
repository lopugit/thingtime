...
test('JSON Action inputs and draft-saving suites negotiate both origin manifests', () => {
	...
	for (const [feature, minimum, previous] of [
		...
		['api.actions-run', '1.17.0', '1.16.0'], ['api.webpages-suites-install', '1.3.0', '1.2.0']
	]) {
...
test('Data Thing controls negotiate the shared-content action contract', () => {
	assert.equal(createApiCapabilitiesManifest().features['api.actions-run'], '1.17.0');
	assert.equal(thingtimeCapabilityManifest('https://thingtime.test').features['api.actions-run'].version, '1.17.0');
