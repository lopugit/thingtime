...
test('emoji transfer, library and move capabilities reject pre-support origins', () => {
  ...
  for (const [feature, version] of [['api.things-export', '1.15.0'], ['api.things-import', '1.11.0'], ['api.things', '1.29.0'], ['api.things-bulk', '1.5.0']]) {
...
test('archive library and folder export reject origins without complete archive traversal', () => {
  const manifest = thingtimeCapabilityManifest('https://thingtime.test');
  for (const [feature, required, previous] of [['api.things', '1.29.0', '1.13.1'], ['api.things-export', '1.15.0', '1.9.0']]) {
  ...
  assert.equal(createApiCapabilitiesManifest().features['api.things'], '1.29.0');
  assert.equal(createApiCapabilitiesManifest().features['api.things-export'], '1.15.0');
});
...
test('shared dependency reads negotiate the additive Things contract on both manifests', () => {
	...
	assert.equal(createApiCapabilitiesManifest().features['api.things'], '1.29.0');
	assert.equal(thingtimeCapabilityManifest('https://thingtime.test').features['api.things'].version, '1.29.0');
	...
	assert.equal(createApiCapabilitiesManifest().features['api.things-update'], '1.6.0');
	assert.equal(thingtimeCapabilityManifest('https://thingtime.test').features['api.things-update'].version, '1.6.0');
...
test('Data Thing controls negotiate the shared-content action contract', () => {
	assert.equal(createApiCapabilitiesManifest().features['api.actions-run'], '1.8.0');
	assert.equal(thingtimeCapabilityManifest('https://thingtime.test').features['api.actions-run'].version, '1.8.0');
...
test('subspace user flairs publish their contract versions', () => {
	...
	assert.equal(manifest.features['api.things'], '1.29.0');
...
test('Lopu continuation clients reject origins without checkpoint and renewable-task contracts', () => {
 ...
 const manifest = thingtimeCapabilityManifest('https://thingtime.test');
 for (const [feature, required, previous] of [['api.actions-run', '1.7.0', '1.5.0'], ['api.things', '1.29.0', '1.19.0'], ['api.things-update', '1.5.0', '1.3.0']]) {
...
test('uncapped post attachments and profile grants negotiate their own feature contracts', () => {
  ...
  for (const [feature, version, previous] of [['api.things', '1.29.0', '1.18.0'], ['api.things-user', '1.6.0', '1.5.0']]) {
...
test('cursor discussion clients require the additive 1.27 contract while media keeps 1.24 compatibility', () => {
  ...
  assert.equal(source.features['api.things'], '1.29.0');
  assert.equal(origin.features['api.things'].version, '1.29.0');
