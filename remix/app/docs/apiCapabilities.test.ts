...
test('standalone Thing copying negotiates the additive copy contract on both manifests', () => {
	...
	for (const unsupported of ['', '1.2.1', '1.3.0', '1.3.1', '2.0.0']) assert.equal(capabilitySatisfies(unsupported, '1.3.2'), false);
	assert.equal(capabilitySatisfies('1.3.3', '1.3.2'), true);
});
...
test('shared audience-boundary corrections reject pre-fix and incompatible origins', () => {
	...
	for (const [name, required, previous] of [
		...
		['actions-run', '1.3.1', '1.3.0'], ['things-fork', '1.3.2', '1.3.1'], ['attachment-content', '1.6.2', '1.6.1']
	]) {
