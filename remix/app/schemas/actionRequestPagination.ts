...
const path = (value: unknown): value is string => typeof value === 'string' && value.length <= 160 &&
	value.split('.').length <= 8 && value.split('.').every((key) => /^[A-Za-z_][A-Za-z0-9_]*$/.test(key) && !['__proto__', 'prototype', 'constructor'].includes(key));
export function parseActionRequestPagination(raw: unknown): ActionRequestPagination | null {
	...
	if (!path(spec.cursorParam) || spec.cursorParam.includes('.') || !path(spec.cursorPath) || !path(spec.itemsPath) ||
		spec.cursorPath === spec.itemsPath || spec.cursorPath.startsWith(spec.itemsPath + '.') || spec.itemsPath.startsWith(spec.cursorPath + '.')) return null;
	if (spec.itemKey !== undefined && !path(spec.itemKey)) return null;
	const maxPages = spec.maxPages ?? 20, maxItems = spec.maxItems ?? 5000;
	if (typeof maxPages !== 'number' || !Number.isInteger(maxPages) || maxPages < 1 || maxPages > 100 ||
		typeof maxItems !== 'number' || !Number.isInteger(maxItems) || maxItems < 1 || maxItems > 10000) return null;
