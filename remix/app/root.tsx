...
const useClientLayoutEffect = typeof document !== 'undefined' ? React.useLayoutEffect : React.useEffect;

const Document = withEmotionCache(function Document({ children, title = 'Thingtime', titlePrefix = '' }: DocumentProps, emotionCache) {
		...
		<html lang="en">
			<head>
				...
				{serverStyleData?.map(({ key, ids, css }) => (
					<style key={`${key}-${ids.join(' ')}`} data-emotion={`${key} ${ids.join(' ')}`} dangerouslySetInnerHTML={{ __html: css }} />
				))}
			</head>
			<body>
				{children}
...
const getDeploymentBranchName = () => {
	return process.env.VERCEL_GIT_COMMIT_REF || process.env.THINGTIME_BRANCH_NAME || 'git/unknown';
};

const shouldShowDeploymentStatus = () => {
	return process.env.NODE_ENV === 'development' || process.env.VERCEL_ENV === 'preview' || process.env.VERCEL_TARGET_ENV === 'preview';
};
...
export async function loader({ request }: { request: Request }) {
	...
	const titlePrefix = process.env.NODE_ENV === 'development' || url.hostname !== 'thingtime.com' ? '🧑‍💻' : '';
	...
	processEnv.THINGTIME_VERCEL_ENV = process.env.VERCEL_TARGET_ENV || process.env.VERCEL_ENV;
