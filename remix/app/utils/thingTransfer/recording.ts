...
export const recordingTransferFile = (thing: TransferThing, manifest: ThingTransfer) => {
  ...
  if (!isTransferRecording(thing) || Object.keys(thing.crystal).length !== 1 ||
    ...
    thing.folderId || thing.targetId || thing.extended != null || thing.tags?.length ||
    ...
    throw new Error('A recording transfer requires exactly one stored file, without folders, gallery links or extra fields');
  }
  return file;
};
