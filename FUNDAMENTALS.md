...
- **Protected** kinds — `user`, `theme`, `feed-algorithm`, `waitlist`, `app`,
  ...
  `account-link`, `migration-diagnostic`, `attachment`, every `ci-*`
  control-plane kind, and the connections family (`external-account`,
  `external-account-link`, `external-post`, `external-post-source`,
  `feed-filter`, `feed-filter-verdict` — a forged link would grant another user's personal
  feed and a forged account could carry attacker tokens) — are
