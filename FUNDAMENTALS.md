...
| `things`   | the actual Thingtime data — feed posts as `kind: 'post'` docs (`/api/v1/things`) and generic typed CRUD records as `kind: 'record'` docs (`/api/v1/crud/records`, ACL-guarded + optionally field-encrypted) |
| `thingTypes` | user-defined schemas for `kind: 'record'` things (`/api/v1/crud/types`; field kinds, required/encrypted/searchable policies, default ACL) |
